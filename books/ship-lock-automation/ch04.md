...
本章以船闸批量放行调度优化为主题，通过仿真模型的构建与求解，系统展示了船闸调度问题的建模思路、优化方法和性能评估体系：

- **随机到港建模**采用泊松过程描述船舶到港的随机性，以指数分布间隔生成船流，符合航运交通流的统计特征，为调度策略的仿真验证提供了真实可靠的输入条件。
- **批量放行调度模型**将闸室容量约束、船型组合与过闸时间结合，构建了"固定间隔批量放行"的决策框架，揭示了放行间隔与等待时间、能耗之间的权衡关系——间隔过短导致频繁操作能耗上升，间隔过长导致等待时间和拥堵风险增加。
- **多目标优化求解**利用标量化目标函数（等待惩罚+能耗惩罚+超时惩罚），将多目标问题转化为单目标连续优化，并通过`scipy.optimize.minimize_scalar`在可行区间内高效搜索最优放行间隔，体现了运筹学在工程调度中的实际应用价值。
- **关键绩效指标（KPI）体系**覆盖效率（平均等待、95分位等待）、服务质量（准点率）和经济性（总能耗、单位能耗）三个维度，其中95分位等待时间比均值更能反映极端拥堵风险，是评估调度鲁棒性的重要指标。
- **基线策略与优化策略对比**通过等待时间CDF曲线和队列长度时序，直观展示了优化调度在降低峰值排队、改善尾部服务质量方面的效果，为工程决策提供量化依据。

## 习题

1. 在本章仿真模型中，船舶到港服从泊松过程，平均到港率为每小时10艘。请计算在一个20分钟放行间隔内，期望到港的船舶数量及其标准差，并分析泊松分布在描述船闸到港随机性方面的适用性与局限性。

2. 过闸时长模型定义为 $T_{cycle} = T_{base} + n_{units} \times T_{per\_unit}$，其中基础时间 $T_{base} = 10$ min，单位附加时间 $T_{per\_unit} = 2.2$ min，闸室容量上限为8个容量单位。当一批过闸船舶恰好装满闸室时，计算该批次过闸时长，并与空载闸室操作时间相比，讨论负载附加时间对船闸通过能力的影响。

3. 分析放行间隔从20分钟缩短至10分钟时，对以下指标的影响趋势：（a）平均等待时间；（b）每小时过闸能耗；（c）闸室平均装载率。请结合"成本-效益"权衡，说明存在最优放行间隔的原因，并用图示方式定性描述目标函数随放行间隔变化的曲线形态。

4. 本章模型采用先进先出（FIFO）的排队规则进行批次组装。在实际三峡船闸运行中，还需考虑船舶优先级（如危险品船、大型客船）和上下行分流。请设计一种改进的排队规则，使紧急船舶能够优先过闸，同时分析该规则对普通船舶平均等待时间的影响。

5. 编程实践：在本章仿真脚本基础上，将固定放行间隔策略扩展为"动态阈值触发"策略（即当等待队列中的容量单位数达到阈值 $N_{trigger}$ 时立即开闸）。分别测试 $N_{trigger} \in \{4, 6, 8\}$ 三种阈值，与最优固定间隔策略进行KPI对比，分析动态触发策略在随机到港场景下的优势与不足。
