...
验收时请专家做了一次可观性分析，结论让所有人吃了一惊：如果把30个监测点换成精心选取的18个点（位置完全不同——集中在弯道、分叉口和末梢），对关键状态的估计覆盖率反而从78%提升到了95%以上。（注：本案例数据为综合多个实际工程经验提炼的典型情景，旨在说明传感器布局优化的一般规律。）
...
沙坪水电站就是时间维度可控性的极端案例：上游来水变化后75～85分钟才传到下游坝前，但调节库容极小（调节库容仅585万方，汛期有效调节库容更小），水位可能5分钟就超标。如果没有提前预判、没有预设的自动响应策略，光靠人反应是来不及的。这就好比你开着一辆刹车距离200米的大卡车在狭窄山路上行驶——你的"可控性"取决于你能不能在200米之前就看到弯道。
...
类似的情况还有泵站功率不够。某调水工程在设计时按"平均用水量"配置了泵站，但夏季高峰期的用水量是平均值的1.5倍——泵站全开也跟不上需求。结果要么下游缺水，要么不得不启用备用线路（如果有的话）。这不是运行人员的问题，是设计阶段就埋下的"可控性缺陷"。CHS提出的MBD方法论（第八章会讲）强调的就是：在设计阶段就做可控可观性分析，而不是建完了再发现"控制力不够"。
...
- 如果红灯项主要集中在可观性——说明你的工程连"看清楚"都做不到，大概在L0（手动运行）到L1（规则自动化）之间。优先加传感器。
- 如果可观性基本过关但可控性有红灯——说明你"看得见但管不住"，大概在L1（规则自动化）到L2（条件自动化）之间。需要增加控制点或引入预测控制。
- 如果可观性和可控性都过关但冗余不足——说明正常工况下能应对，但一旦设备故障就会降级。大概在L2（条件自动化）到L3（条件自主）的门槛上。需要加冗余。
...
短期能做的是"用信息换时间"——利用预测控制（MPC）技术[3-3]，提前预判来水变化，在问题还没传播到"死区"之前就在上游提前采取行动。打个比方：你的车刹车距离很长（可控性弱），那你就必须看得更远（用预测弥补），提前减速。沙坪水电站就是用这个思路——调节库容极小（"刹车距离"短），但用MPC预测未来75～85分钟的来水变化[3-4]，提前调整闸门，相当于"把刹车提前踩了"。
...
- **传感器质量胜于数量**：30个随机布点的传感器可能远不如18个优化选址的传感器——关键在覆盖"信息密集区"（弯道、分叉口、控制断面），而非均匀铺开；"全覆盖"不等于"可观性好"。
- **体检四步流程**：列清单→查可观性→查可控性→查冗余，每步都有具体的检验方法和评分标准，产出红黄绿三色体检报告，直接对应工程整改优先级。
- **处方的核心逻辑**：可观性不足优先考虑传感器优化选址或状态估计算法（软测量），而非简单加装更多传感器；可控性不足优先考虑MPC（用信息换时间），而非直接扩建硬件。
- **体检结果与WNAL直接对应**：红灯项对应当前等级的能力短板，体检报告是制定WNAL升级路线图的基础文件。

## 习题

1. 某配水管网有20个节点，目前在其中8个节点安装了压力传感器（均匀间距布置）。请解释为什么"均匀间距"不是最优的传感器布置策略，并说明如何用可观性分析方法找到更优的8个布置位置。

2. 某渠道系统可观性分析显示：中游一段6公里渠道内没有任何监测点，状态估计误差达35%。请设计一个补充方案：（1）新增最少数量的监测点使估计误差降至10%以下；（2）或采用状态估计算法（软测量）在不新增传感器的条件下改善可观性，比较两种方案的成本和效果。

3. 可控性不足（"方向盘不灵"）在水利工程中有哪些典型表现？请列举三个具体场景，分析可控性不足的原因，并提出相应的改善措施。

4. 请为一个典型的梯级调水工程（3个渠池串联，2个泵站，1个分水口）完整执行"体检四步"，假设你可以自行设定初始传感器配置和控制设备情况，给出一份模拟的红黄绿三色体检报告，并提出整改建议。
