...
> - 约束传播（上游进黄区即触发下游预警）可显著减少梯级系统的水位越限事件（工程经验表明可减少70%以上），并大幅减少闸门的"急操作"次数，延长设备寿命。
...
- **安全包络vs.报警系统**：传统报警系统是"信号灯"（通知人来处理），安全包络是"ABS+安全气囊"（在人来不及之前自动介入）；安全包络是CHS八原理中优先级最高的机制，是系统迈向L3的必要条件。
- **三区机制的分级响应**：绿区追求效率最大化（MPC优化无约束），黄区自动切换保守策略并尽力恢复（不优化、先保安全），红区执行预定义确定性保护动作（不计算、先保命）；三区响应策略截然不同，不能混用。
- **约束传播放大保护效果**：上游节点进入黄区即触发下游预警，使梯级系统在水位越限事件发生之前就开始响应，将被动应急转变为主动防控，越限事件可减少70%以上。
...
1. 本章开篇故事中，"报警响了12次，没人管"是典型的"报警疲劳"现象。请分析：（1）该问题的系统性根源是什么（不仅仅是"值班员不负责任"）？（2）安全包络机制如何从根本上解决这一问题？（3）引入安全包络后，值班员的工作内容会如何变化？

2. 某调水工程设计了红黄绿三区安全包络：绿区水位52.0-53.5m，黄区52.0以下或53.5-54.0m，红区54.0m以上或51.5m以下。请设计三区对应的具体控制响应逻辑，包括黄区的"保守策略"内容和红区的"确定性保护动作"内容。

3. 约束传播机制要求"上游进黄区即触发下游预警"。请分析这一机制在以下场景中的效果：（1）3个渠池串联的调水系统；（2）1个主干渠+3条支渠的配水系统；并指出约束传播可能产生误触发（上游正常波动触发下游不必要响应）的条件和应对策略。
