...
老张苦笑了一下："你按下发电指令，4秒后机组出力就变了。我开了闸门，下游的水位变化——等一个多小时以后再看吧。你那是打乒乓球，我这是打高尔夫——球飞出去之后，只能看着。"
...
这是水系统和电力系统最本质的区别。电力以光速传播，控制指令发出后几乎瞬时生效。水在渠道里以每秒一两米的速度流动——听起来不慢，但渠道动辄几十、几百公里长。南水北调中线从丹江口到北京，全长1432公里，在典型设计流量条件下，水力传播时间约15天[2-2]。胶东调水工程从引黄济青枢纽到威海，水流传播也要好几天[2-5]。即使是规模较小的沙坪水电站，上游枕头坝到沙坪的水力传播时间也有75到85分钟。
...
>
> 提示词：三行横向条形图。第一行"电力系统"条形极短（毫秒级），配闪电图标。第二行"化工系统"中等（分钟级），配化学瓶图标。第三行"水系统"极长（小时至天级），配水波图标。每行标注具体数量级。右侧注释："时滞越大，对预判能力的要求越高。"底部补充具体案例：沙坪水电站75～85分钟、胶东调水数天、南水北调中线15天。蓝绿色调。
...
本章深入分析了水系统区别于电网、工厂等其他复杂系统的五个"先天特质"，以及CHS八原理与这些特质的对应关系：

- **大时滞是核心挑战**：水从上游到下游需要数十分钟乃至数小时，调度必须"提前开牌"——这是MPC成为核心手段的根本原因，也是"水网调度不能靠反应，只能靠预测"的工程逻辑所在。
...
- **强不确定性是常态而非异常**：来水预测误差30-40%是正常水平，传感器故障和数据缺失是日常运营的一部分；鲁棒控制和不确定性量化不是锦上添花，而是工程必须。
- **人机共治是本质而非过渡状态**：跨部门协调（防洪、发电、供水、电网）使水利调度本质上是一个社会治理问题，纯技术方案注定落地困难，人机协作和可解释决策是长期必要需求。

## 习题

1. 电力调度实现了全自动AGC（4秒一次），而水利调度至今仍大量依赖人工判断。请从"大时滞"和"强不确定性"两个角度，系统分析两个行业自动化水平差距的根本原因。

2. 某渠道系统由5个渠池串联，各渠池调节库容均为50万方。上游来水突增200m³/s，试估算：（1）水位越限到达最末一个渠池需要多少时间？（2）如果调度员在感知到首个渠池水位异常后才开始反应，最末渠池是否还有充裕的处置窗口？

3. 大渡河梯级电站因"三个独立AGC各自局部最优"而产生系统性低效。请用博弈论语言（囚徒困境模型）描述这一问题的结构，并说明引入"梯级协调层"在博弈论意义上如何改变了博弈结构。

4. 水网调度涉及防汛、发电、供水、电网调峰等多个利益主体。请设计一个多目标协调机制，说明如何在存在目标冲突时确定优先顺序，并给出一个该机制在实际工程中的应用示例。

5. CHS八原理与水系统五个先天特质一一对应。请从中选取两组对应关系，详细解释为什么该特质需要对应的那条原理来应对，以及如果忽略该原理可能导致的工程后果。
