...
- **预演** = Cyber 空间的 XIL 验证环路。在 MIL/SIL/HIL/OIL 四层验证环境中（参见 《建模》第 13 章），对候选控制策略进行快速仿真校验，确保其在虚拟环境中不会导致安全包络（第 6 章）被击穿。
...
步长不能小于模型的有效分辨率。当步长 $\Delta t$ 小于模型的时间分辨率 $\Delta t_{\text{model}}$ 时，模型输出在相邻步长之间不包含新信息，闭环退化为伪闭环。参见 《建模》第 12 章 Theorem 3 关于模型精度与控制性能的定量关系。
...
功能模拟接口（FMI, Functional Mock-up Interface）标准为异构模型的耦合提供了工业级解决方案（参见 《建模》第 13 章）。在四预工程中，每个模型被封装为独立的 FMU（Functional Mock-up Unit），通过标准化的输入 / 输出端口实现耦合。
...
> **本章后续进阶**：闭环四预的理论论证见《水控》§8.1（CPSS闭环耦合 vs 开环串行的辨析）；HydroOS对四预的Skill级实施见《水控》§13.4；AI驱动的四预优化（强化学习、LLM Agent）见《认知》第 5—7 章。
