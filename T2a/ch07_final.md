...
**MPC的时间步内闭环本质与四预的对应关系**　MPC在每个采样时刻执行的"预测→优化→执行第一步→观测→更新"循环，本质上就是T1第八章§8.1.8所述的闭环四预在实时尺度上的工程实现：预测模型的前向推演=预报，约束集中的ODD边界检查=预警，对候选控制序列的可行性评估=预演，输出的最优序列首元=预案。换言之，**MPC不仅是一种控制算法，更是步长级四预闭环的数学实体化**。这一对应关系使得"四预"概念从宏观调度层面的规划工具，下沉为实时控制层面的内嵌机制——每个控制周期都在做一次微型的预报-预警-预演-预案闭环。
...
$$
\text{HDMPC} = \{\text{L1局部PI}\} + \{\text{L2区域MPC} \times M\} + \{\text{L3全局协调}\} \tag{7-43}
$$
