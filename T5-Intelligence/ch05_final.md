...
v2 2026-04-28: 阶段二格式刷齐——"操作设计域"→"运行设计域"、跨卷引用按concept_authority v2.1对齐
v3 2026-04-28: 阶段三格式刷齐——补充本变更日志
-->

# 第5章 强化学习调度：从仿真到真实水网的迁移

> **本章定位**：《认知》第 5 章已系统讲授了RL在水网中的适用性、与MPC的互补关系以及RL的基本算法类别（value-based/policy-based/actor-critic）。本章不重复《认知》的原理内容，而是聚焦算法工程师在实际项目中面临的工程问题：PPO vs SAC vs DQN在水网场景中怎么选、超参怎么调、Sim-to-Real怎么解决、安全约束怎么嵌入。本章是"RL算法工程师的水网部署实战手册"。

> **本章前置阅读**：《认知》第 5 章 RL原理与适用性——理解本章讨论的算法对比的前提；《建模》第 7 章 MPC——理解RL与MPC互补关系的工程基础；《水控》§10.3 WNAL权威分级定义——渐进式部署的自主等级框架。
...
MPC基于物理模型的在线滚动优化（详见《建模》第7章），擅长在分钟至小时级时域内精确处理水力约束与安全包络，但其性能高度依赖模型精度，且预测时域受限于模型可信度。RL基于数据与经验的策略学习，擅长在小时至天级时域内处理长时域多目标权衡与不确定性决策，但难以提供硬约束保证。两者的互补性天然适合分层架构：**RL在上层生成调度策略或目标参考值，MPC在下层跟踪执行并处理实时约束**。
...
从CPSS（Cyber-Physical-Social System）视角审视，这种分工具有深层逻辑：RL处理的是Cyber与Social界面的不确定性——用水需求波动、多利益方目标权衡、极端气候下的策略调整；MPC处理的是Cyber与Physical界面的确定性控制——水力方程约束、设备安全包络、实时流量跟踪。这一分层结构与《建模》第12章所述的HDC（Hierarchical Distributed Control）分层架构高度一致：RL承担L2协调层的角色，负责跨时段、跨子系统的目标协调；MPC承担L1过程层的角色，负责单子系统内的实时最优控制。§5.6.2将进一步讨论该架构在HydroOS中的具体部署实现。
...
运行设计域（Operational Design Domain, ODD）定义了RL策略可安全运行的状态空间边界。ODD的关键监测指标包括三类：
...
> **本章后续进阶**：《平台》第 4 章（步长级仿控耦合——RL策略在L2内核中的实时部署架构）、《平台》第 9 章（Agent Fabric——RL Agent作为Agent Fabric中的决策Agent类型）、《标治》第 6 章（安全包络工程标准——RL策略的约束合规性验证）。
