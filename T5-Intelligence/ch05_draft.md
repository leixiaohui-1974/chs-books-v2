...
$$\pi^* = \arg\max_{\pi} \mathbb{E}_{\pi}\left[\sum_{t=0}^{T} \gamma^t \mathcal{R}(s_t, a_t, s_{t+1})
\right]$$
...
$$\phi_{\text{season}}(t) = \left[\sin!\left(\frac{2\pi t}{365}
\right), \cos!\left(\frac{2\pi t}{365}
\right)
\right]$$
...
$$r_{\text{smooth}} = -\lambda_{\text{smooth}} \sum_{i=1}^{m} \left(a_{i,t} - a_{i,t-1}
\right)^2$$
...
$$r_{\text{supply}} = \min!\left(\frac{Q_{\text{supply},t}}{D_{\text{demand},t}}, 1
\right) - \eta \cdot \max!\left(1 - \frac{Q_{\text{supply},t}}{D_{\text{demand},t}}, 0
\right)$$
...
$$L^{\text{CLIP}}(\theta) = \mathbb{E}_t\left[\min!\left(r_t(\theta)\hat{A}_t, \text{clip}(r_t(\theta), 1-\epsilon, 1+\epsilon)\hat{A}_t
\right)
\right]$$
...
\rho^{\pi}\left[\mathbb{E}_{a \sim \pi(\cdot|s)}\left[Q(s,a) - \alpha \log \pi(a|s)
\right]
\right]$$

评论家网络更新（使用软目标 $\phi'$ 稳定训练）：

$$L_Q(\phi) = \mathbb{E}\left[\left(Q_\phi(s,a) - \left(r + \gamma\mathbb{E}_{a'}\left[Q_{\phi'}(s',a') - \alpha \log \pi(a'|s')
\right]
\right)
\right)^2
\right]$$
...
$$L_\alpha(\alpha) = -\alpha\,\mathbb{E}_{a \sim \pi}!\left[\log \pi(a|s) + H_0
...
$$y = r + \gamma\min_{i=1,2}Q_{\phi'_i}!\left(s', \mu_{\theta'}(s') + \epsilon
\right), \qquad \epsilon \sim \text{Clip}!\left(\mathcal{N}(0, \sigma), -c, c
\right)$$
...
$$L_Q = \mathbb{E}!\left[\left(Q_{\text{global}}(\mathbf{s}, \mathbf{a}) - \left(r + \gamma Q'_{\text{global}}(\mathbf{s}', \mathbf{a}')
\right)
\right)^2
\right]$$
...
$$Q_{\text{tot}}(\mathbf{s}, \mathbf{a}) = f_{\text{mix}}!\left(Q_1(s_1,a_1), \ldots, Q_n(s_n,a_n); \mathbf{s}
\right)$$
...
其中 $J_{C_i}(\pi) = \mathbb{E}\left[\sum_t \gamma^t c_i(s_t, a_t)
ight]$ 为约束期望累积成本，$d_i$ 为允许阈值。
...
Lagrangian增广奖励（参见5.5.1节推导）：$	ilde{R}(s,a;oldsymbol{\lambda}) = R(s,a) - \sum_{k=1}^{5} \lambda_k c_k(s,a)$。乘数更新轨迹：训练前200万步 $\lambda_k$ 快速上升，700万步后稳定收敛于区间 $[0.3, 1.8]$。
...
$$lpha_t = rac{lpha_0}{\sqrt{t}} = rac{3 	imes 10^{-4}}{\sqrt{t}}$$

$$D_{	ext{KL}}(P_t \| P_0) = \sum_{s,a} P_t(s,a) \log rac{P_t(s,a)}{P_0(s,a)} < 0.1 \quad 	ext{（上限约束）}$$
