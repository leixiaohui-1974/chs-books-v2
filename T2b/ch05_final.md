...
### 5.1.4 RL与MPC的对偶性：Bellman方程的共同根基

"补充而非替代"的关系可以从数学上得到更深刻的理解。RL和MPC并非两种不相干的技术，它们共享同一个数学根基——**Bellman最优性原理**。

MPC在每个控制时刻求解一个有限时域优化问题：$\min_{u_0,\ldots,u_{N-1}} \sum_{k=0}^{N-1} \ell(x_k, u_k) + V_f(x_N)$，其中终端代价 $V_f$ 即为Bellman值函数的近似。MPC的本质是：**已知模型，在线求解Bellman方程的有限时域近似**。

RL则从另一端出发：**不假设模型已知**，通过与环境（或仿真器）的反复交互，用采样数据逼近Bellman方程的解——无论是Q-learning直接估计最优Q函数（§5.3），还是Actor-Critic用策略梯度间接优化值函数（§5.4），核心都是求解同一个Bellman方程。
...
| **Bellman方程求解方式** | 在线有限时域优化 | 离线采样+函数逼近 |
| **策略生成时机** | 每个控制时刻实时求解 | 离线训练，在线查表/前向推理 |
| **对模型精度的依赖** | 强（模型偏差→控制偏差） | 弱（从真实交互中学习） |
| **对数据量的依赖** | 弱（需要模型，不需要海量数据） | 强（需要大量样本） |
| **可解释性** | 强（约束和目标显式） | 弱（策略是黑箱） |

这一对偶性揭示了水系统控制的自然分工：**MPC适合模型精确、工况可预见的实时闭环控制（宏观尺度的确定性工作流）；RL适合模型不确定、工况多变的策略学习（跨场景的灵活决策）。** 在工程实践中，两者最有效的协作模式是时间尺度分离：RL在宏观尺度（天至周）学习调度策略（如何分配水量、如何设定目标水位），MPC在实时尺度（分钟至小时）执行闭环控制（如何跟踪RL设定的目标）。这正是CPSS框架中Cyber空间内部"规划层+执行层"分层的技术实现。
