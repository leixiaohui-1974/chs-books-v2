...
> **卷间分工提示**：本章讲述RL在水网中的适用性、与MPC的机理-学习互补关系、RL的基本算法类别。关于具体RL算法（PPO vs SAC vs DQN）的工程选型、超参调优、线上部署实践，详见《算法》第 5 章。前置阅读：[《水控》第 6 章, ch14]、《建模》第 7 章。
...
> 上一章通过 PINN 将物理方程嵌入学习过程，解决了"预测要符合物理"的问题。但预测只是手段，最终目的是**决策**——在已知或预测的水系统状态下，确定最优的控制动作。传统控制方法（《建模》 详述的 PID/MPC 等）需要显式模型和解析目标函数。本章引入强化学习（RL），它通过与环境的试错交互直接学习策略，特别适合模型不完备、目标动态变化、多目标冲突的复杂场景。在 CHS 框架（Lei 2025a）中，RL 是认知 AI 引擎的"决策学习"模块——物理 AI 引擎提供仿真环境和安全约束，RL 在此约束下学习超越手工规则的调度策略。

![图5-1: DQN与PPO强化学习架构对比](./H/fig_05_01_rl_architecture.png)

> 图 5-1: DQN（离散动作空间）与 PPO（连续动作空间）强化学习架构在水系统控制中的对比
...
> **核心概念**（10 个）：马尔可夫决策过程（MDP）、价值函数、Q 函数、Bellman 方程、经验回放、策略梯度、PPO 截断代理目标、SAC 最大熵、约束 MDP（CMDP）、最小风险状态。
...
《建模》 介绍的控制方法（PID、LQR、MPC）共享一个范式：**基于模型的解析优化**。给定系统模型 $\dot{\mathbf{x}} = f(\mathbf{x}, \mathbf{u})$ 和目标函数 $J = \sum \ell(\mathbf{x}, \mathbf{u})$，通过数学规划求解最优控制序列 $\mathbf{u}^*$。
...
在 CHS 双引擎架构中，RL 的角色是**在物理 AI 引擎提供的仿真环境中，为认知 AI 引擎学习复杂决策策略**：





- **物理 AI 引擎提供环境**：数字孪生仿真器（《建模》第十三章）充当 RL 的"训练环境"，状态转移由水力学模型驱动


- **认知 AI 引擎集成策略**：RL 学到的策略嵌入多智能体系统（ch07），作为 Agent 的决策内核
...
### 5.1.4 RL 与 MPC 的对偶性：Bellman 方程的共同根基

"补充而非替代"的关系可以从数学上得到更深刻的理解。RL 和 MPC 并非两种不相干的技术，它们共享同一个数学根基——**Bellman 最优性原理**。

MPC 在每个控制时刻求解一个有限时域优化问题：$\min_{u_0,\ldots,u_{N-1}} \sum_{k=0}^{N-1} \ell(x_k, u_k) + V_f(x_N)$，其中终端代价 $V_f$ 即为 Bellman 值函数的近似。MPC 的本质是：**已知模型，在线求解 Bellman 方程的有限时域近似**。

RL 则从另一端出发：**不假设模型已知**，通过与环境（或仿真器）的反复交互，用采样数据逼近 Bellman 方程的解——无论是 Q-learning 直接估计最优 Q 函数（§5.3），还是 Actor-Critic 用策略梯度间接优化值函数（§5.4），核心都是求解同一个 Bellman 方程。

两者的对偶关系可以精确表述为：

| 维度 | MPC | RL |
|------|-----|-----|
| **模型假设** | 模型已知（白箱/灰箱） | 模型未知（黑箱交互） |
| **Bellman 方程求解方式** | 在线有限时域优化 | 离线采样+函数逼近 |
| **策略生成时机** | 每个控制时刻实时求解 | 离线训练，在线查表/前向推理 |
| **对模型精度的依赖** | 强（模型偏差→控制偏差） | 弱（从真实交互中学习） |
| **对数据量的依赖** | 弱（需要模型，不需要海量数据） | 强（需要大量样本） |
| **可解释性** | 强（约束和目标显式） | 弱（策略是黑箱） |

这一对偶性揭示了水系统控制的自然分工：**MPC 适合模型精确、工况可预见的实时闭环控制（宏观尺度的确定性工作流）；RL 适合模型不确定、工况多变的策略学习（跨场景的灵活决策）。** 在工程实践中，两者最有效的协作模式是时间尺度分离：RL 在宏观尺度（天至周）学习调度策略（如何分配水量、如何设定目标水位），MPC 在实时尺度（分钟至小时）执行闭环控制（如何跟踪 RL 设定的目标）。这正是 CPSS 框架中 Cyber 空间内部"规划层+执行层"分层的技术实现。
...
RL 训练需要大量的状态-动作-奖励样本。在水系统中，直接在真实系统上训练（在线 RL）不可行——一次错误的探索可能导致溃坝或城市停水。因此，水系统 RL 的训练环境是**数字孪生仿真器**（参见 《建模》第十三章）。
...
以及动作价值函数（Q 函数）$Q^\pi(s, a)$ 为在状态 $s$ 执行动作 $a$ 后遵循 $\pi$ 的期望累积奖励：
...
**水系统中的 DQN 应用**：DQN 适合离散动作空间的水系统控制，如泵站启停组合优化（$N$ 台泵对应 $2^N$ 种组合）、闸门档位控制（每个闸门 5~10 个离散档位）。DQN 的神经网络逼近器本质上是深度学习的函数逼近能力（Goodfellow et al., 2016 第 6 章）在强化学习中的应用。
...
[表 5-8: PPO vs SAC——动作空间]

| 动作空间 | 连续/离散 | 连续为主 |



[表 5-9: PPO vs SAC——样本效率]

| 样本效率 | 中低 | 高 |



[表 5-10: PPO vs SAC——训练稳定性]

| 训练稳定性 | 高（截断保证） | 高（双 Q 网络） |



[表 5-11: PPO vs SAC——探索能力]

| 探索能力 | 一般 | 强（熵正则化） |



[表 5-12: PPO vs SAC——超参数]

| 超参数 | 较少 | 较少（自动$\alpha$） |



[表 5-13: PPO vs SAC——推荐场景]
...
[表 5-15: MPC vs RL——模型依赖]

| 模型依赖 | 需要显式模型 | 无需显式模型（需仿真环境） |



[表 5-16: MPC vs RL——求解方式]

| 求解方式 | 每步在线优化 | 离线训练 + 在线推理 |



[表 5-17: MPC vs RL——在线计算]

| 在线计算 | 高（解 QP/NLP） | 低（前向传播） |



[表 5-18: MPC vs RL——约束处理]

| 约束处理 | 显式硬约束 | 需额外约束机制（§5.6） |



[表 5-19: MPC vs RL——可解释性]

| 可解释性 | 高（有模型和目标） | 低（黑箱策略） |



[表 5-20: MPC vs RL——时间尺度]

| 长时间尺度 | 受预测时域限制 | 可通过折扣因子调节 |



[表 5-21: MPC vs RL——多目标适应]
...
在 RL 策略之外设置独立的安全监督器（参见 《建模》第十章安全包络）：
...
**切换逻辑**：ODD 分类器（可基于 ch02 的机器学习方法或 ch06 的认知 AI 引擎）判断当前工况类别，自动选择对应策略。切换时使用"软过渡"——两个策略的输出按权重混合，权重在数个控制周期内平滑过渡，避免动作突变。
...
- **L4**：RL 自主执行所有已验证 ODD 内的决策，人工仅在 ODD 外介入
...
步骤 1：状态设计——$s_t = [V_t/V_{\max}, Q_{\text{in},t}/Q_{\text{in,max}}, \hat{Q}_{t+1:t+7}/Q_{\text{in,max}}, \text{month}_t/12]$，共 10 维（归一化库容 + 归一化入流 + 7 天预报 + 季节标识）。
...
下一章将进入认知 AI 引擎——大语言模型与认知智能，讨论如何将专家知识、文档语义与实时运行状态整合为可交互的调度认知能力。
...
> **本章后续进阶**：《算法》第 5 章（RL算法对比与选型——PPO vs SAC vs DQN的工程实践）、《平台》第 4 章（步长级仿控耦合——RL策略在L2内核中的实时部署架构）。
...
- **MDP 建模框架**：水系统调度问题可统一用马尔可夫决策过程描述，状态空间设计应涵盖水位、流量、闸门开度及时间特征，奖励函数需平衡安全、效率和设备损耗多个目标。


- **算法选型原则**：离散控制空间选 DQN 系列，连续控制空间选 PPO（稳定性强，工程首选）或 SAC（样本效率高，适合在环训练），两类算法均需经过充分调参和验证。


- **约束强化学习是工程必要条件**：水系统存在水位限制、流量约束和设备保护等硬约束，必须采用 CMDP 框架或投影梯度等方法显式处理约束，不能依赖奖励惩罚项软处理。


- **分级上线是工程规范**：RL 策略上线需经历离线训练→MIL 验证→SIL→HIL→影子运行→分级授权→一键回退的完整链路，每一步都需通过门禁指标才能进入下一阶段。


- **物理 AI 提供安全环境**：物理 AI 引擎构建的仿真环境和安全包络为 RL 提供训练沙箱和运行边界，是 RL 策略安全可靠的根本保障。
