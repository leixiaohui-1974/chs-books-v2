...
v2 2026-03-02: 从骨架版(5.6k字)全面扩写至~3万字；新增xIL体系定位(§10.1)、MIL/SIL/HIL/OIL详解(§10.2)、测试用例设计方法(§10.3)、覆盖度体系(§10.4)、缺陷分级与回归(§10.5)、ODD-安全包络联动验证(§10.6)、测试基础设施(§10.7)、5例题、15习题、25篇参考文献
...
2. 区分 MIL/SIL/HIL/OIL 四级在环测试的目标、边界、成本与适用场景；
...
> **章首衔接（承接 ch09）**
> 上一章给出了 HydroOS 水网操作系统的系统化落地框架——设备抽象层、调度引擎、认知 API 和工作台层构成了自主水网的软件骨架。但一个核心问题尚未回答：**如何证明这套系统可以安全上线？** 功能正确不等于运行安全，离线测试通过不等于现场可靠。本章建立在环测试（In-the-Loop Testing, xIL）的系统化方法，通过分层验证逐步缩小"仿真与现场"的差距，使策略在上线前暴露问题、收敛风险。Lei 2025c 提出的在环测试体系为本章提供了方法论基础。
...
>
> **核心概念**（8 个）：在环测试（xIL）、MIL、SIL、HIL、OIL、测试覆盖度、发布门禁、缺陷分级。
...
$$
\text{MIL} \xrightarrow{\text{+真实软件}} \text{SIL} \xrightarrow{\text{+真实硬件}} \text{HIL} \xrightarrow{\text{+真实环境}} \text{OIL} \tag{10-1}
$$
...
### 10.1.4 XIL 作为四预的工程实现

《水控》 第八章§8.1.8 建立了"基于 CHS 的四预闭环体系"——预报、预警、预演、预案不是四个串行环节，而是每个时间步内的闭环耦合。在这一框架中，**XIL 验证体系恰好对应"预演"环节的工程实现**，但其含义远比"测试手段"更为丰富。

**XIL 四级 = 预演四层**

| XIL 级别 | 对应预演层面 | 验证内容 | 时间约束 |
|---------|-----------|---------|---------|
| MIL（模型在环） | 预演的模型层 | 多方案并行仿真，筛选可行域，确认不违反安全包络 | 实时尺度：须在一个控制周期内完成 |
| SIL（软件在环） | 预演的软件层 | 验证控制算法代码在真实软件环境下的行为一致性 | 准实时：可在部署前批量执行 |
| HIL（硬件在环） | 预演的硬件层 | 验证控制指令在真实 PLC/RTU 硬件上的响应特性 | 准实时：需要物理设备在线 |
| OIL（调度员在环） | 预演的人员层 | 验证人机界面的可用性、调度员的认知负荷和决策时效 | 小时级：需要人类参与 |

传统观点把 XIL 视为"上线前的测试手段"——跑一轮 MIL/SIL/HIL 通过了就可以部署。但在 CHS 四预闭环体系中，XIL 的角色发生了本质变化：

**宏观尺度 XIL（离线预演）**：在日、周、月的时间尺度上，XIL 用于规划阶段的方案比选。MIL 仿真不同调度策略的长期效果，SIL 验证软件更新的兼容性，HIL 测试设备改造后的响应特性。这是传统意义上的"测试"——在部署前发现问题。

**实时尺度 XIL（在线预演）**：在分钟至小时的时间尺度上，XIL 嵌入运行系统内部，成为每个决策时间步的一部分。每当 MPC 求解器生成一个候选控制序列，系统在执行前先用 MIL 快速预演该序列的后果（水位轨迹是否越限？边坡水位变化率是否安全？），预演通过才允许执行。这不是"测试"，而是**闭环控制的内嵌验证环节**——"先预演再执行"成为实时控制回路的确定性工作流步骤。

**实时 MIL 的工程约束**：在线预演要求 MIL 仿真速度满足硬实时约束——仿真计算时间必须远小于控制周期Δt。对于典型的 15 分钟控制周期，MIL 需要在秒级内完成多个候选方案的并行评估。这对降阶模型的计算效率提出了严格要求（参见 《建模》第四章 IDZ 模型和第十三章数字孪生平台），也解释了为什么在线预演只能使用降阶模型而非高保真 Saint-Venant 求解。

**多模态运行下的 XIL 策略切换**：当系统运行模态从正常（G0）升级至警戒（G2）或应急（G3）时（ch08§8.2.8），XIL 预演的策略随之调整：MIL 并行评估的方案数增加（从 3 个扩展到 10+个）、SIL 验证的场景矩阵加密、HIL 测试的设备覆盖范围扩大。在极端模态（G4）下，系统可能降级为跳过在线预演、直接执行 MRC（最小风险状态）的确定性降级链路——这是"确定性工作流在极端场景下兜底"的又一体现。

### 10.1.5 国内外在环测试实践概述

在环测试最初源于航空航天和汽车工业。Isermann et al.（1999）系统总结了硬件在环仿真在机电系统开发中的应用。DO-178C（RTCA, 2011）是航空软件验证的国际标准，定义了从 A（灾难性）到 E（无影响）的五级软件保证等级（DAL），每个等级对应不同的测试和文档要求。ISO 26262（2018）为汽车功能安全定义了 ASIL A-D 四级，与在环测试深度绑定。

水利行业在在环测试方面起步较晚。Buyalski（1991）在渠道系统自动化手册中记录了早期的控制器现场测试方法。Van Overloop（2006）在明渠 MPC 控制研究中使用了 MIL 验证。Clemmens et al.（2005）在美国垦务局的灌区自动化项目中引入了 SIL 测试。Litrico & Fromion（2009）的水系统建模框架为 MIL 提供了标准仿真平台。ASCE MOP 131（2014）在灌溉系统自动化指南中推荐了分阶段验证流程，但系统化的 MIL-SIL-HIL-OIL 全流程在水利行业尚未形成标准。Lei 2025c 的贡献在于提出了面向水系统的完整在环测试体系框架。

---
![图10-1: xIL四级测试金字塔](./H/fig_10_01_xil_test_levels.png)

> 图 10-1: 在环测试四级金字塔（MIL→SIL→HIL→OIL），保真度与成本递增


## 10.2 MIL/SIL/HIL/OIL 分层框架
...
- 系统模型（Saint-Venant 方程离散化、IDZ 传递函数、管网节点模型等，参见 《建模》第二至四章）
- 参数敏感性（模型参数偏差对控制性能的影响）

**实施环境**：
- 仿真软件：MATLAB/Simulink、Python + SciPy、OpenModelica（参见 《建模》第十三章数字孪生平台）
...
**（5）联锁功能验证**：在模拟危险场景下，验证 PLC 硬件联锁是否正确触发并独立于软件控制系统。Leveson（2011）强调，安全联锁必须是独立于控制软件的硬件级保护。

**HIL 实施环境**：
- 被控对象：实时仿真器（需实现 1:1 时间比例的仿真）或物理实验装置（如水槽、微缩管网）
...
### 10.2.4 OIL（Operator-in-the-Loop，运行在环）

**定义**：控制系统与真实的物理水系统及人类操作员构成闭环，在实际工程环境中进行影子运行或受控试运行。

**OIL 的特殊性**：与航空/汽车的在环测试不同（可以在封闭试验场进行），水系统的 OIL 必须在运行中的实际工程上进行——因为水系统不能"停下来测试"。因此，OIL 通常采用两种模式：
...
$$
\text{总缺陷发现效率} = \sum_{l \in \{\text{MIL,SIL,HIL,OIL}\}} \frac{N_l^{\text{defects}}}{C_l^{\text{cost}}} \tag{10-2}
$$
...
|:---|:---|:---|:---|:---|
| MIL | 1-2 周 | 1× | 算法逻辑、参数不当 | 40-50% |
| SIL | 2-4 周 | 3-5× | 接口错误、时序问题、异常处理 | 25-30% |
| HIL | 4-8 周 | 10-20× | 硬件约束、通信问题、联锁 | 15-20% |
| OIL | 1-3 月 | 50-100× | 运行环境适应性、人机交互 | 5-10% |

**关键洞见**：约 70-80% 的缺陷可以在 MIL 和 SIL 阶段发现，成本仅为 HIL/OIL 的 1/10。因此，充分的 MIL/SIL 测试是控制总成本的关键。Boehm & Basili（2001）的研究证实，缺陷修复成本随发现阶段呈指数增长。
...
四维度各 4 级的全组合为 $4^4 = 256$ 个场景——对于一个泵站群系统可能是可管理的规模，但对于大型调水系统（多个子系统、多个 ODD），全组合测试不现实。
...
安全包络（Safety Envelope，参见 《建模》第十章）定义了系统运行的绝对约束边界。在环测试必须验证安全包络在所有条件下都能正确执行：
...
**已知**：某明渠调水系统包含 5 个闸站，采用分布式 MPC 控制（参见 《建模》第七章）。需要在 MIL 阶段验证控制策略的基本有效性。
...
| TC-P1-01 | 常态 | 1 闸检修 | 标准 | P1 |
| TC-P1-02 | 阶跃 | 1 闸检修 | 标准 | P1 |
| TC-P1-03 | 常态 | 正常 | n+20% | P1 |
| TC-P1-04 | 常态 | 正常 | n-20% | P1 |
| TC-P2-01 | 阶跃 | 1 闸检修 | n+20% | P2 |
...
**已知**：某区域调水系统已通过 SIL 和 HIL 验证，进入 OIL 影子运行阶段。系统采用 MAS 多智能体调度（ch07），在线生成调度建议但不实际执行。
...
| 指标 | 计算方法 | 达标标准 |
|:---|:---|:---|
| 建议一致率 | AI 建议与人工操作方向一致的比例 | ≥ 85% |
| 建议优越率 | AI 建议预期效益优于人工操作的比例 | ≥ 60% |
| 安全合规率 | AI 建议满足全部安全约束的比例 | = 100% |
| 响应及时率 | AI 建议在规定时间内给出的比例 | ≥ 98% |
| ODD 感知率 | 正确识别 ODD 边界事件的比例 | ≥ 95% |
...
1. **在环测试是自主运行的"准入门票"**。不经验证不上线，不通过门禁不发布。xIL 体系通过 MIL→SIL→HIL→OIL 的逐级递进，将验证逼近真实运行条件。

2. **四级测试各有分工**。MIL 验证算法逻辑（成本低、速度快），SIL 验证软件集成（接口、时序、异常处理），HIL 验证硬件约束（通信、联锁、执行器），OIL 验证运行适应性（影子运行、分级放权）。约 70-80% 的缺陷在 MIL/SIL 阶段发现。
...
**10-1.** MIL、SIL、HIL、OIL 四级在环测试的核心差异是什么？各级主要验证什么类型的缺陷？
...
**10-5.** 解释"影子运行"和"分级放权"两种 OIL 模式的适用场景。
...
**10-10.** 为某调水系统的 OIL 影子运行设计 30 天评估方案，包括评估指标、数据采集方案和决策规则。

### 思考题

**10-11.** 在环测试能否保证系统 100% 安全？如果不能，还需要哪些补充手段？

**10-12.** 水系统的 OIL 测试必须在运行中的实际工程上进行。这带来哪些特殊挑战？如何管理 OIL 阶段的风险？
...
- **xIL 的核心地位**：在环测试是自主水网"不经验证不上线"原则的工程实现，通过 MIL→SIL→HIL→OIL 的分层验证逐步缩小仿真与现场的差距，是 WNAL 等级跃迁的必要门禁。
- **四级测试的分工**：MIL 验证算法逻辑正确性（成本低、可快速迭代），SIL 验证代码实现与运行时行为，HIL 暴露硬件集成与时序问题（能发现 SIL 无法发现的缺陷），OIL 在实际工程环境中验证运行适应性与人机交互。
- **测试用例设计**：场景矩阵覆盖正常工况、边界工况、故障工况和极端工况四类，测试用例应由水文专家、控制工程师和安全工程师协同设计，覆盖 ODD 边界附近的关键场景。
- **覆盖度量化**：场景覆盖率、风险覆盖率和回退覆盖率三项指标量化测试充分性，缺一不可；只追求场景数量而忽视风险覆盖率是常见的测试误区。
- **证据驱动的发布机制**：版本发布须基于测试报告（证据）而非主观判断，缺陷按 A/B/C 分级处理，A 类缺陷一票否决，回归测试覆盖所有修复项，确保"修一个、不引入一个新的"。
