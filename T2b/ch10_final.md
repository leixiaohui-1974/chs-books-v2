...
T1 第八章§8.1.8 建立了"基于 CHS 的四预闭环体系"——预报、预警、预演、预案不是四个串行环节，而是每个时间步内的闭环耦合。在这一框架中，**XIL 验证体系恰好对应"预演"环节的工程实现**，但其含义远比"测试手段"更为丰富。
...
**实时 MIL 的工程约束**：在线预演要求 MIL 仿真速度满足硬实时约束——仿真计算时间必须远小于控制周期Δt。对于典型的 15 分钟控制周期，MIL 需要在秒级内完成多个候选方案的并行评估。这对降阶模型的计算效率提出了严格要求（参见 T2a 第四章 IDZ 模型和第十三章数字孪生平台），也解释了为什么在线预演只能使用降阶模型而非高保真 Saint-Venant 求解。
