...
>
> **核心概念**（8个）：在环测试（xIL）、MIL、SIL、HIL、OIL、测试覆盖度、发布门禁、缺陷分级。
...
T1第八章§8.1.8建立了"基于CHS的四预闭环体系"——预报、预警、预演、预案不是四个串行环节，而是每个时间步内的闭环耦合。在这一框架中，**XIL验证体系恰好对应"预演"环节的工程实现**，但其含义远比"测试手段"更为丰富。

**XIL四级 = 预演四层**

| XIL级别 | 对应预演层面 | 验证内容 | 时间约束 |
|---------|-----------|---------|---------|
| MIL（模型在环） | 预演的模型层 | 多方案并行仿真，筛选可行域，确认不违反安全包络 | 实时尺度：须在一个控制周期内完成 |
| SIL（软件在环） | 预演的软件层 | 验证控制算法代码在真实软件环境下的行为一致性 | 准实时：可在部署前批量执行 |
| HIL（硬件在环） | 预演的硬件层 | 验证控制指令在真实PLC/RTU硬件上的响应特性 | 准实时：需要物理设备在线 |
| OIL（调度员在环） | 预演的人员层 | 验证人机界面的可用性、调度员的认知负荷和决策时效 | 小时级：需要人类参与 |

传统观点把XIL视为"上线前的测试手段"——跑一轮MIL/SIL/HIL通过了就可以部署。但在CHS四预闭环体系中，XIL的角色发生了本质变化：

**宏观尺度XIL（离线预演）**：在日、周、月的时间尺度上，XIL用于规划阶段的方案比选。MIL仿真不同调度策略的长期效果，SIL验证软件更新的兼容性，HIL测试设备改造后的响应特性。这是传统意义上的"测试"——在部署前发现问题。

**实时尺度XIL（在线预演）**：在分钟至小时的时间尺度上，XIL嵌入运行系统内部，成为每个决策时间步的一部分。每当MPC求解器生成一个候选控制序列，系统在执行前先用MIL快速预演该序列的后果（水位轨迹是否越限？边坡水位变化率是否安全？），预演通过才允许执行。这不是"测试"，而是**闭环控制的内嵌验证环节**——"先预演再执行"成为实时控制回路的确定性工作流步骤。

**实时MIL的工程约束**：在线预演要求MIL仿真速度满足硬实时约束——仿真计算时间必须远小于控制周期Δt。对于典型的15分钟控制周期，MIL需要在秒级内完成多个候选方案的并行评估。这对降阶模型的计算效率提出了严格要求（参见T2a第四章IDZ模型和第十三章数字孪生平台），也解释了为什么在线预演只能使用降阶模型而非高保真Saint-Venant求解。

**多模态运行下的XIL策略切换**：当系统运行模态从正常（G0）升级至警戒（G2）或应急（G3）时（ch08§8.2.8），XIL预演的策略随之调整：MIL并行评估的方案数增加（从3个扩展到10+个）、SIL验证的场景矩阵加密、HIL测试的设备覆盖范围扩大。在极端模态（G4）下，系统可能降级为跳过在线预演、直接执行MRC（最小风险状态）的确定性降级链路——这是"确定性工作流在极端场景下兜底"的又一体现。
...
水利行业在在环测试方面起步较晚。Buyalski（1991）在渠道系统自动化手册中记录了早期的控制器现场测试方法。Van Overloop（2006）在明渠MPC控制研究中使用了MIL验证。Clemmens et al.（2005）在美国垦务局的灌区自动化项目中引入了SIL测试。Litrico & Fromion（2009）的水系统建模框架为MIL提供了标准仿真平台。ASCE MOP 131（2014）在灌溉系统自动化指南中推荐了分阶段验证流程，但系统化的MIL-SIL-HIL-OIL全流程在水利行业尚未形成标准。Lei 2025c 的贡献在于提出了面向水系统的完整在环测试体系框架。

---
n![图10-1: xIL四级测试金字塔](./H/fig_10_01_xil_test_levels.png)

> 图10-1: 在环测试四级金字塔（MIL→SIL→HIL→OIL），保真度与成本递增
...
| OIL | 1-3月 | 50-100× | 运行环境适应性、人机交互 | 5-10% |
...
- **xIL的核心地位**：在环测试是自主水网"不经验证不上线"原则的工程实现，通过MIL→SIL→HIL→OIL的分层验证逐步缩小仿真与现场的差距，是WNAL等级跃迁的必要门禁。
- **四级测试的分工**：MIL验证算法逻辑正确性（成本低、可快速迭代），SIL验证代码实现与运行时行为，HIL暴露硬件集成与时序问题（能发现SIL无法发现的缺陷），OIL在实际工程环境中验证运行适应性与人机交互。
