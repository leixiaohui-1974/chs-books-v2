...
2. 描述 HydroOS 三层架构（设备抽象层、物理AI引擎层、认知AI引擎层）的层间职责与接口规范；
...
HydroOS 采用三层架构，从下到上分别是设备抽象层（DAL）、物理AI引擎层（HydroCore）、认知AI引擎层（HydroClaw）。
...
### 9.2.3 认知AI引擎层（HydroClaw）

**目标**：将 HydroCore 的计算和智能能力以服务形式暴露给用户和外部系统。
...
三层架构的层间关系可以用一句话概括：**DAL 解决"接什么"，HydroCore 解决"算什么"，HydroClaw 解决"给谁用"**。
...
在**架构层面**，HydroOS 采用三层架构：设备抽象层（DAL）统一异构设备、物理AI引擎层（HydroCore）运行计算和决策、认知AI引擎层（HydroClaw）对外提供服务。三层之间通过标准化接口解耦，实现"一次开发，多处复用"。
...
2. 三层架构的层间关系：DAL 解决"接什么"，HydroCore 解决"算什么"，HydroClaw 解决"给谁用"；
