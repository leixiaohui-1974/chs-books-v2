...
从 CPSS 框架（T1 第八章）审视，多智能体系统不仅是"把集中控制器拆成多个"的技术手段，更是**CPSS 三空间分布式耦合的自然形态**。
...
**混用的危险**：当灵活编排入侵 Physical 空间时（例如 LLM Agent 直接控制闸门），确定性保证丧失，安全风险急剧上升。当确定性编排入侵 Social 空间时（例如强制调度员按固定流程操作不允许灵活判断），系统将失去对未预见场景的适应能力。CHS 的设计原则是：**每种编排方式在其适用的 CPSS 空间内运行，接口处有明确的安全包络约束**——HydroCore 的安全否决权（T1 第十四章）正是这一接口的工程实现。
...
为了帮助读者把 MAS 与 T2a/T2b 的知识体系对齐，同时理解它与 WNAL 自治等级的关系，可参照表 7-2 的映射。每一类 Agent 都有明确的物理 AI 底座（T2a 提供的确定性能力）和认知 AI 增强（T2b 提供的快慢思考能力），共同决定系统能否从 L2 辅助决策跃迁到 L3 条件自主。

[表 7-2: MAS Agent 类型与 T2a/T2b 能力映射]
| MAS 层/Agent 类型 | HydroClaw 示例 | T2a 基础能力（物理 AI） | T2b 增强能力（认知 AI） | 对 WNAL 的作用 |
|------------------|----------------|-------------------------|-------------------------|----------------|
