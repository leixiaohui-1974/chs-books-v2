...
### 7.1.3 多智能体的CPSS视角：三空间的分布式耦合

从CPSS框架（T1第八章）审视，多智能体系统不仅是"把集中控制器拆成多个"的技术手段，更是**CPSS三空间分布式耦合的自然形态**。

在实际大型水网中，不同智能体可能分属不同的CPSS空间：Physical空间中的PLC控制器是reactive agent（毫秒级硬保护）；Cyber空间中的MPC优化器是planning agent（秒至分钟级协调控制）；Cyber↔Social界面上的LLM Agent是general agent（分钟至小时级语义推理和方案解释）。MAS的编排问题，本质上是**跨空间、跨时间尺度的智能体协调问题**。

这一视角引出了一个关键的工程设计原则：**确定性编排与灵活编排的区分**。

- **确定性编排**（Physical空间主导）：闸门的操作序列、泵站的启停时序、安全降级的执行链路——这些涉及物理执行的多智能体协调必须采用确定性编排。步骤不可跳过、顺序不可乱、超时必须有兜底。§7.6将要介绍的DAG（有向无环图）编排正是确定性编排的技术实现。
- **灵活编排**（Social空间主导）：调度方案的多方会商、异常事件的因果分析、跨部门的方案协调——这些涉及语义理解和人机交互的多智能体协调可以采用灵活编排。Agent可以根据运行时信息动态调整推理策略、引入新的Agent参与、或将决策权上交人类。

**混用的危险**：当灵活编排入侵Physical空间时（例如LLM Agent直接控制闸门），确定性保证丧失，安全风险急剧上升。当确定性编排入侵Social空间时（例如强制调度员按固定流程操作不允许灵活判断），系统将失去对未预见场景的适应能力。CHS的设计原则是：**每种编排方式在其适用的CPSS空间内运行，接口处有明确的安全包络约束**——HydroCore的安全否决权（T1第十四章）正是这一接口的工程实现。
