...
## 代码示例：Actor-Critic 单闸门水位控制

以下代码展示了高斯策略 Actor-Critic 算法在单闸门水位控制中的应用。Actor 输出连续动作（闸门开度），Critic 用 TD 误差评估状态价值。

```python
import numpy as np

np.random.seed(7)
A, dt, Qin, k_gate = 100.0, 1.0, 8.0, 12.0
h_ref, gamma = 5.0, 0.95
lr_a, lr_c = 0.01, 0.05

# Actor: mu = wa*h + ba, sigma固定；Critic: V = wc*h + bc
wa, ba = np.random.randn() * 0.1, 0.0
wc, bc = np.random.randn() * 0.1, 0.0
sigma = 0.12  # 高斯策略标准差（固定）

def env_step(h, u):
    h2 = h + (Qin - k_gate * u) * dt / A
    r = -((h2 - h_ref) ** 2 + 0.01 * u * u)
    return h2, r

for ep in range(5):  # 演示回合
    h = np.random.uniform(2.0, 8.0)
    for t in range(25):
        mu = wa * h + ba
        a_raw = mu + sigma * np.random.randn()      # 高斯采样动作
        u = np.clip(a_raw, 0.0, 1.0)                # 闸门开度约束
        h2, r = env_step(h, u)

        # TD误差：delta = r + gamma*V(s') - V(s)
        v, v2 = wc * h + bc, wc * h2 + bc
        delta = r + gamma * v2 - v
        ...
        # Actor更新：theta += lr * delta * grad(log pi(a|s))
        g = (a_raw - mu) / (sigma ** 2)
        wa += lr_a * delta * g * h
        ba += lr_a * delta * g
    ...
    print(f"ep={ep}, h={h:.3f}, mu={wa*h+ba:.3f}, V={wc*h+bc:.3f}")
```
