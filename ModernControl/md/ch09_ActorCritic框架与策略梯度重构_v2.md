...
|------|----------------|---------------------|
| 学习对象 | $Q^*(s,a)$ → 隐式策略 | $\pi_\theta(a|s)$ → 直接输出动作 |
| 动作空间 | 仅离散 | 离散/连续均可 |
...
| 特征 | DDPG（第8章） | A2C（本章） |
|------|-------------|------------|
| 策略类型 | 确定性 $\mu_\theta(s)$ | 随机 $\pi_\theta(a|s) = \mathcal{N}(\mu, \sigma^2)$ |
...
**工程部署要点**：A2C/PPO的随机策略在训练阶段提供内生探索能力（无需外加噪声），部署时可通过取均值 $\mu_\theta(s)$ 转化为确定性策略。输出仍须经过安全层校验（第7章7.6.3节），确保不违反汛限水位等硬约束。在CHS框架中，DRL智能体（A2C/PPO）→安全层→执行器的三级决策链，实现了"认知AI提议→物理AI验证→安全层兜底"的架构（Lei 2025b）。
