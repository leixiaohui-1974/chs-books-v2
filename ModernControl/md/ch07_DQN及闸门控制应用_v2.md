...
运行上述训练代码并使用配套脚本（`scripts/ch07_dqn_reservoir.py`）生成仿真结果，可以得到以下三组关键图形。

![图7-1 DQN训练奖励曲线](../figures/ch07_dqn_training.png)

**图7-1** DQN训练奖励曲线。左侧浅色线为每轮（episode）的原始累计奖励，右侧深色线为20轮滑动平均。可以观察到奖励从初始的负值区间逐步上升并趋于稳定，反映了智能体从随机探索到有效控制策略的学习过程。

![图7-2 DQN闸门控制效果](../figures/ch07_dqn_control.png)

**图7-2** DQN训练完成后的控制效果（最终评估轮次）。蓝色实线为水位变化过程，橙色实线为闸门开度变化过程。红色虚线标注汛限水位（150 m），绿色虚线标注目标水位（145 m），橙色虚线标注死水位（130 m）。

![图7-3 DQN水库状态综合可视化](../figures/ch07_dqn_reservoir.png)

**图7-3** DQN水库调度综合仿真结果。包含训练奖励曲线和最终控制效果的综合展示，完整呈现了从训练收敛到策略部署的全过程。

#### 7.6.3.1 仿真设置概述

仿真采用单库水库模型，库容面积 $A_s = 5 \times 10^6$ m²，模拟周期为30天（720个时间步，每步1小时）。入库流量包含基流（300 m³/s）和一个高斯型洪峰过程（峰值约800 m³/s，出现在第15天附近，持续约4天），叠加标准差为30 m³/s的随机扰动。DQN智能体训练200轮，使用两层全连接网络（各64个神经元），经验回放缓冲区容量10000，目标网络每100步硬更新一次，$\epsilon$ 从1.0按指数衰减至0.01。

#### 7.6.3.2 训练过程分析

从图7-1的训练奖励曲线可以观察到以下特征：

1. **初始探索阶段**（前30~50轮）：$\epsilon$ 值较高，智能体以较大概率随机选择动作，累计奖励波动剧烈且普遍偏低。这一阶段的主要作用是填充经验回放缓冲区，为后续学习积累多样化的状态-动作-奖励样本。

2. **快速提升阶段**（第50~120轮）：随着 $\epsilon$ 衰减和Q网络参数的持续更新，智能体逐步学会了基本的水位控制策略——在洪水来临前预泄、洪峰期加大泄量。奖励曲线呈现明显的上升趋势。

3. **收敛稳定阶段**（第120轮以后）：$\epsilon$ 降至较低水平，策略趋于稳定。滑动平均奖励不再显著提升，表明Q网络已基本逼近最优Q函数。此时偶尔出现的奖励波动主要来自入库流量的随机扰动。

#### 7.6.3.3 控制性能分析

从图7-2和图7-3的控制效果图中可以得出以下结论：

**水位控制方面**：训练后的DQN智能体能够将水位维持在安全范围内——水位始终未超过汛限水位（150 m），也未跌破死水位（130 m）。在非洪峰时段，水位基本稳定在目标水位（145 m）附近。

**洪峰应对方面**：DQN学习到了合理的洪峰应对策略。在洪峰到达前（约第13天起），闸门开度开始逐步增大，进行预泄腾库；洪峰期间（第14~16天），闸门开度进一步增大以加速泄洪；洪峰过后，闸门开度逐渐回调，水位平稳回落至目标水位附近。这一调度策略与有经验的水库管理者的操作模式高度相似。

**操作平稳性方面**：由于动作空间为离散7档（最大变化幅度为$\pm 10\%$），闸门开度变化呈阶梯状。奖励函数中的操作平稳性惩罚项有效地抑制了不必要的频繁调节——智能体学会了在非紧急时段优先选择"保持不变"（动作索引3）或微调（$\pm 2\%$）动作。

#### 7.6.3.4 工程意义

本案例的仿真结果揭示了DQN在水库闸门调度中的几个工程特点：

1. **无模型学习的可行性**：DQN在不知道水量平衡方程具体参数的情况下，仅通过与仿真环境的交互即学会了有效的调度策略，验证了无模型强化学习在水利调度中的可行性。

2. **离散动作的适用性**：7档离散动作虽然限制了控制精度，但对于闸门调度这类决策频率相对较低（小时级）的场景，离散化带来的精度损失在可接受范围内。实际工程中，许多闸门的控制系统本身就采用档位式操作。

3. **安全层的必要性**：尽管DQN在训练后期学会了安全操作，但在训练初期的随机探索阶段，水位可能短暂越限。这说明在实际部署中，必须在DQN输出端叠加基于规则的安全层（详见下节），确保任何情况下都不违反硬约束。

#### 7.6.3.5 与其他方法对比

DQN作为离散动作空间的深度强化学习基线方法，其性能和特性可与后续章节的方法进行对比：

- **与传统分级调度图对比**：DQN学习到的策略在应对随机扰动时表现出更强的自适应性，能够根据实时水情动态调整闸门开度，而非按固定规则操作。
- **与DDPG（第8章）的差异**：DQN的离散动作空间导致控制曲线呈阶梯状，DDPG的连续动作空间可实现更平滑的闸门调节，适合对控制精度要求更高的场景。
- **与A2C（第9章）的差异**：DQN的离策略学习（经验回放）使其样本效率较高，但不具备A2C随机策略的内生探索能力和方差控制优势。
