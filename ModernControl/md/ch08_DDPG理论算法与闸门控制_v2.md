...
| 特征 | 随机策略 $\pi_\theta(a|s)$ | 确定性策略 $\mu_\theta(s)$ |
|------|---------------------------|--------------------------|
| 输出 | 动作的概率分布 | 唯一确定的动作 |
| 探索 | 内生（采样随机性） | 需外加噪声 |
...
**工程部署要点**：DDPG/TD3/SAC的策略输出仍须经过安全层校验（第7章7.6.3节的safety_layer函数），确保不违反汛限水位等硬约束。在CHS框架中，DRL扮演"认知AI提议"角色，安全层扮演"物理AI验证"角色（Lei 2025b）。
