...
仿真结果表明，AMPC通过在线更新内部模型，能在旁侧取水扰动后更快地恢复水位，且稳态误差更小。PI控制器虽然结构简单，但面对模型参数时变时性能明显下降。
...
仿真结果表明：
1. MRAC控制下的液位能精确跟踪参考模型输出，即使水箱参数时变；
2. 控制器参数 $\theta_1(t), \theta_2(t)$ 随跟踪误差自适应调整，最终收敛到稳定值；
3. PI控制器在设定值变化时表现出明显的性能差异（有的超调大，有的响应慢），而MRAC的响应始终与参考模型一致。
