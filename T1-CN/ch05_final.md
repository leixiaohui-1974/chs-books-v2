...
3. **实现**基于RLS或EKF的在线参数辨识流程，并解释其递推更新机理；
...
这样，可以直接对线性系统 $\dot{H} = v$ 设计线性控制器（如PID或LQR），再通过上式反解实际出库流量指令。

**局限性与工程建议**：

1. $A(H)$ 必须实时可测或可准确估计（通过库容曲线）；当 $H \to H_{sill}$（死水位）时，$A(H) \to 0$，逆变换趋于奇异，需设置保护逻辑
2. $Q_{in}(t)$ 为入库流量，需要实时量测或短期预报值
3. **工程主线建议**：对于大多数水利工程，增益调度或线性参数变化（LPV）MPC是更稳健的选择；反馈线性化在此主要作为理论参考，帮助读者理解非线性控制的基本思路
...
本节的不确定性分析是第四章建模基础与第十章ODD定义、第十一章安全包络之间的连接纽带：**ODD的约束类(C)和鲁棒性(R)边界由不确定性量化支撑，而水力(H)、输入(I)和环境(E)边界由物理极限和运行规程确定**[5-11]。
