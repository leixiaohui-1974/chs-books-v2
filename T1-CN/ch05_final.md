...
1. **识别**水系统中的主要非线性来源，并选择合适的局部线性化或非线性补偿方法；
2. **设计**面向多工况运行的增益调度策略，并给出调度变量与切换逻辑；
3. **实现**基于 RLS 或 EKF 的在线参数辨识流程，并解释其递推更新机理；
4. **评估**辨识模型在噪声与扰动条件下的收敛性、鲁棒性与实时性表现；
5. **量化**模型参数与外部扰动不确定性对系统响应和控制性能的影响范围。
...
> 
> **与后章的关系**：本章的非线性处理、不确定性量化为第七章"水系统控制论八原理"（含原理三分层分布式控制）和第十一章"安全包络与在环验证"提供建模基础
> 
> **核心概念**（6 个）：
...
2. 在每个工况点解析计算 ID 参数 $(A_s^{(i)}, \tau_d^{(i)})$，$i = 1, \ldots, K$；
...
反馈线性化是将非线性系统通过状态变换和控制变换转化为线性系统的技术。以水库水位控制为例，水量平衡方程为：

$$
A(H) \frac{dH}{dt} = Q_{in}(t) - Q_{out}(t)
$$

其中 $A(H)$ 为水位 $H$ 对应的库面面积（非线性函数），$Q_{in}$ 为入库流量（外部扰动），$Q_{out}$ 为出库流量（控制量）。

**线性化方法**：引入虚拟控制量 $v = dH/dt$，则水位动态变为标准线性积分器：
...
这样，可以直接对线性系统 $\dot{H} = v$ 设计线性控制器（如 PID 或 LQR），再通过上式反解实际出库流量指令。

**局限性与工程建议**：

1. $A(H)$ 必须实时可测或可准确估计（通过库容曲线）；当 $H \to H_{sill}$（死水位）时，$A(H) \to 0$，逆变换趋于奇异，需设置保护逻辑
2. $Q_{in}(t)$ 为入库流量，需要实时量测或短期预报值
3. **工程主线建议**：对于大多数水利工程，增益调度或线性参数变化（LPV）MPC 是更稳健的选择；反馈线性化在此主要作为理论参考，帮助读者理解非线性控制的基本思路
...
含时滞系统的连续域近似采用 Padé近似（$s$域有理逼近）处理指数项 $e^{-\tau_i s}$：一阶 Padé 近似为 $e^{-\tau s} \approx (1 - \tau s/2)/(1 + \tau s/2)$，二阶近似为 $(1 - \tau s/2 + \tau^2 s^2/12)/(1 + \tau s/2 + \tau^2 s^2/12)$，在 $|\tau s|  \lt  1$ 范围内误差小于 2%。数字实现中，若纯时延为采样周期整数倍，应采用状态增广法（$z^{-d}$移位寄存器）实现精确离散化。
...
**层次化 MPC（Hierarchical MPC）**：分为协调层（计算全局负荷分配、水量分配目标）和执行层（各站实现局部 MPC）。协调层求解慢时间尺度（15–60 分钟）全局优化问题，向执行层发送设定点；执行层求解快时间尺度（1–10 分钟）局部跟踪控制。这正是第十三章 HydroOS 三层架构中物理 AI 引擎层（HydroCore）的多策略融合结构的对应体现。
...
§4.5.2 的解析公式给出了 ID/IDZ 参数的理论值，然而实际工程中模型参数随时间漂移是常态：渠道淤积改变过水断面，冰期糙率系数变化，闸门磨损导致流量系数偏移，季节性水温影响黏度……这些变化使基于设计参数辨识的"一次定稿"模型逐渐失准。
...
其中 $\theta$ 为待辨识参数向量，$\varphi(k)$ 为回归向量，$\varepsilon(k)$ 为噪声。注意，纯时延参数 $\tau_d$ 不能直接作为线性回归参数——时延的变化使回归向量本身发生结构变化。工程中通常采用两步法：**第一步**，将 ID 模型离散化为 ARX（或 FIR）结构，辨识离散系数 $\theta_{ARX} = [a_1, b_0, b_1, \ldots]^\top$；**第二步**，由辨识得到的离散系数反推物理参数 $\hat{A}_s$ 和 $\hat{\tau}_d$。对于时延变化场景，也可采用"固定时延网格 + 多模型并行辨识 + 最小残差选择"的方案。
...
**典型应用效果**（说明性设定）：在类似调水渠道中引入在线辨识（RLS, $\lambda = 0.98$）后，MPC 在冰期的水位控制误差可从 ±8cm 量级降至 ±3cm 量级，自动控制可用率（无需人工干预的比例）可从约 70% 提升至约 90%。具体效果取决于工程条件和辨识数据质量。
...
**自适应 MPC 完整流程**：综合上述在线辨识、精度监控和模型切换机制，形成自适应 MPC 的完整算法框图（见图 5-4）。该框图展示了从数据采集、参数辨识、模型验证到控制器更新的闭环流程，是 CHS 原理六（认知增强）和原理八（自主演进）在建模层面的具体实现。

![图5-4: 自适应MPC框图](./H/fig_05_04_adaptive.png)

> 图 5-4: 自适应 MPC 完整算法框图
...
![图5-5: 不确定性传播链](./H/fig_05_05_uncertainty.png)
...
![图5-6: 约束收紧策略](./H/fig_05_06_constraint.png)
...
本节的不确定性分析是第四章建模基础与第十章 ODD 定义、第十一章安全包络之间的连接纽带：**ODD 的约束类(C)和鲁棒性(R)边界由不确定性量化支撑，而水力(H)、输入(I)和环境(E)边界由物理极限和运行规程确定**[5-11]。

当工况条件（如流量、水深、季节）使得模型不确定性 $\Delta\theta$ 超过鲁棒性设计所能容忍的最大值 $\Delta\theta_{max}$ 时，系统已经超出 ODD，应触发 MRC（最小风险状态）切换。这将在第十一章安全包络框架中进一步定义和实现。
...
| **运行** | 在线残差监控 | RMSE < 阈值 $\epsilon_{alarm}$| §5.3.4 建模精度评估 |
...
1. **主模型**：IDZ/ID 传递函数。参数 $(A_s, \tau_d, \tau_m)$ 通过§4.5.2 的解析公式或渠道阶跃响应辨识获得[5-4]。
...
| **时间尺度** | 典型应用 | 推荐模型 | 关键参数 |
|---------|---------|---------|---------|
| **分钟级（1–15 min）** | 水库水位/闸位调节 | ID 传递函数 | 渠池/库区蓄水面积 $A_s$，传输延迟 $\tau_d$；对于 AGC 频率调节，需使用水轮机-引水系统的机组线性模型（见第八章）|
...
![图5-7: 建模策略决策树](./H/fig_05_07_decision.png)

> 图 5-7: 面向不同水系统的建模策略决策树
...
> **算例性质声明**：§5.6 的工程参数和应用效果数据为**说明性设定**，旨在演示管网传递函数辨识的完整流程，不对应特定工程的实测数据。参数量级参照多个城市供水管网的公开文献设定，具有工程合理性。

![图5-8: 管网拓扑](./H/fig_05_08_network.png)

> 图 5-8: 城市供水管网拓扑

### 5.6.1 工程背景

某城市供水管网，服务面积约 45 $\text{km}^2$，管网总长数百 km，设 10 座泵站和 50 个压力监测点。
...
本章与第四章共同构成"水系统形式化建模"的完整知识体系。第四章提供基础工具（状态空间、传递函数、ID/IDZ），本章应对复杂场景（非线性、耦合、不确定性）。后续章节将在此基础上展开控制设计（第七章）、安全包络与在环验证（第十一章）。

---

## 附录 5-A 与在环验证和工程案例的联系

本章的高级建模技术在第十一章"安全包络与在环验证（xIL）"中得到全面应用[5-15]：
