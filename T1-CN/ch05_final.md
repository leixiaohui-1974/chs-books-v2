...
**梯级水电站的站间耦合**：如第十六章大渡河梯级案例所示，上游电站出力调整 → 下泄流量变化 → 经时延 $\tau_{ij}$ 到达下游电站 → 改变下游电站坝前水位 → 影响水头和出力。这种耦合构成一个有向图，上游电站的任何调整都以延迟方式向下游传播。
...
在线辨识（Online Identification）解决的核心问题是：**在不中断正常运行的前提下，利用运行数据实时更新模型参数**[5-7]，使 MPC 控制器始终基于接近真实的模型工作。这是 CHS 原理六（认知增强）和原理八（全生命周期自主演进）在建模层面的直接体现。正如雷晓辉等[5-1]从学科发展角度所指出的，水资源系统分析正在经历从"静态平衡"到"动态控制"的范式转变，而在线自适应建模正是这一转变在模型层面的核心支撑技术。
...
本节的不确定性分析是第四章与第七至八章（ODD / 安全包络）之间的连接纽带：**运行设计域（ODD）的外边界，本质上是由不确定性量化结果决定的**[5-11]。
...
水系统涵盖灌渠、调水干线、水库群、城市管网、梯级水电站等多种工程形态，各类系统在主导动力学特征和控制需求上存在显著差异。本节归纳各类系统的建模策略选择准则，为第十五章至第十七章的工程案例提供理论对接。

### 5.5.1 明渠灌区：ID/IDZ 优先策略

明渠灌区的控制目标以水位跟踪为主，渠池动力学符合 IDZ 模型的适用条件（低弗劳德数、小扰动、准稳态）[5-5]。建模策略：

1. **主模型**：IDZ/ID 传递函数。参数 $(A_s, \sigma, \tau)$ 通过§4.5.2的解析公式或渠道阶跃响应辨识获得[5-4]。
...
> **典型案例参照**：南水北调中线总干渠 23 个渠池，每池独立 IDZ 辨识，串联 MPC 控制，已在第十七章案例中详细展开。

### 5.5.2 调水长距离管道：时延主导策略

长距离有压输水管道（引调水工程）的控制特性与明渠有根本差异：水击波速 $c_w = \sqrt{E/\rho}$（弹性波速，量级 1000 m/s）远大于明渠波速（量级 3–8 m/s），主导动力学是**时延 + 液压耦合**[5-12]，而非积分特性。建模策略：
...
> **典型案例参照**：胶东调水工程泵站-管道-平衡池系统，第十六章详细分析时延补偿策略。
...
梯级水电站的建模还需额外处理**站间水力耦合**：上游电站出力调整→引起下泄流量变化→经过传播时延后影响下游水位。第十六章详细分析大渡河梯级的站间时序补偿模型。
...
本章与第四章共同构成"水系统形式化建模"的完整知识体系。第四章提供基础工具（状态空间、传递函数、ID/IDZ），本章应对复杂场景（非线性、耦合、不确定性）。后续章节将在此基础上展开控制设计（第七章）、安全包络（第九章）和在环验证（第九章）。
...
**工程案例关联**：
- 第十七章"胶东调水工程"：§6.5.1 长距离明渠建模策略的实践
...
[5-1] 雷晓辉, 许慧敏, 何中政, 张峥. 水资源系统分析学科展望：从静态平衡到动态控制[J]. 南水北调与水利科技(中英文), 2025, 23(04): 770-777.
