...
**层次化 MPC（Hierarchical MPC）**：分为协调层（计算全局负荷分配、水量分配目标）和执行层（各站实现局部 MPC）。协调层求解慢时间尺度（15–60 分钟）全局优化问题，向执行层发送设定点；执行层求解快时间尺度（1–10 分钟）局部跟踪控制。这正是第十三章 HydroOS 三层架构中调度与智能层（SIL）的多策略融合结构的对应体现。
