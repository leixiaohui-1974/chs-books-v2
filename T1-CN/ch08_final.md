...
1. **构建**涵盖水电站引水系统、机组、发电机与励磁环节的CPSS统一动态模型；
2. **解释**多物理耦合条件下水力子系统与电力子系统的相互作用机理；
3. **建立**基于统一模型的MPC控制问题，并明确目标函数与约束集合；
4. **验证**闭环系统满足Lyapunov稳定性条件，并据此判断运行安全裕度；
5. **综合**性能、稳定性与执行器限制，提出CPSS协调控制的参数整定方案。
...
### 8.1.4 CPS信息侧→物理侧闭环与在环验证

#### (a) 完整闭环的两条链路

CPSS框架的本质是一个**信息-物理深度耦合的闭环系统**。这一闭环包含两条方向相反但缺一不可的链路：

**感知链（Physical→Cyber）**：物理量通过传感器采样、A/D转换、通信传输、数据质控、状态估计，最终更新Cyber层的数字孪生状态。对水电站而言，这条链路将水头、流量、功率、频率等物理量转化为调速器/励磁控制器可使用的状态估计值。

**执行链（Cyber→Physical）**：优化求解生成控制指令，经安全校验后通过通信下发、D/A转换，最终驱动执行器完成物理动作。对水电站而言，这条链路将MPC算法的导叶开度指令转化为伺服机构的实际动作，进而改变水轮机输入功率。

传统SCADA系统只有感知链而缺乏完整的执行链（依赖人工下发指令），本质上是开环的；而传统单点PID只有窄带反馈，无法支撑多目标协调。CPSS框架的创新在于：建立**宽感知（多物理量、多时间尺度）× 深执行（多层优化、多约束校验）**的完整闭环，使系统能够在无需人工干预的条件下应对负荷突变、水头变化等动态工况。

#### (b) 信息侧与物理侧的状态同步问题

CPS闭环的核心挑战在于**信息侧状态与物理侧状态的实时同步**。在稳定工况下，状态同步误差可控；但在快速动态过程（如水击压力波、电网频率突变）中，如果信息侧的状态估计落后于物理侧的实际状态，控制器将基于过时信息做出决策，导致控制性能劣化甚至失稳。

8.1.2节引导案例中的响应滞后（T=2s至T=5s），根源正是Cyber层（传统PID调速器）未能实时追踪Physical层（水击压力波$T_w=2.5$s）的状态变化——控制器“看到的世界”与“物理世界”之间存在不可接受的信息时延。

CPSS框架通过两个机制解决状态同步问题：

1. **短时预报替代纯测量**：Cyber层不仅使用当前传感器读数，还基于Saint-Venant方程预测未来$\Delta t$内的物理状态，使控制器基于“预测状态”而非“历史测量”做决策（详见第四章步长级仿控耦合机制）。

2. **分级时钟对齐**：通过NTP/PTP协议实现毫秒级时钟同步，确保感知链和执行链的时间戳一致性。时钟偏差超过阈值时，系统自动降级为保守控制模式，避免基于时序错误数据做出错误决策。

#### (c) 在环验证：SIL与HIL的工程意义

CPSS闭环能否在极端工况下可靠工作，不能仅靠数学分析或仿真证明，必须通过**在环测试（xIL, x-in-the-Loop）**逐级验证。这一测试体系是第十一章的核心内容，此处先建立基本概念：


**表8-5**

| 验证层级 | 核心问题 | 在CPSS中的意义 | 对应WNAL |
|---------|---------|--------------|---------|
| **SIL（软件在环）** | 控制代码在真实CPU上的执行确定性 | 验证Modbus超时处理、数据帧错误恢复等通信链路的代码逻辑 | L2验证门槛 |
| **HIL（硬件在环）** | 完整控制柜与物理仿真器的闭环行为 | 发现AD转换精度不足、继电器切换延迟等纯软件仿真无法发现的问题 | L3验证门槛 |
| **全系统集成测试** | 控制柜与真实物理装置的联合调试 | 验证水力-电气耦合的实际动态特性 | L4认证 |

**SIL的不可替代性**：水利控制系统的SIL测试必须覆盖以下场景，这些场景在纯算法仿真中无法有效测试：Modbus通信超时（通信链路故障时，控制器应在$T_{\text{timeout}}$内切换到保守设定点）、传感器故障报警处理（编码器断线时，调速器应从闭环切换到开环保守模式）、断电重启状态恢复（控制器上电后，应在$T_{\text{init}}$内完成状态初始化并进入安全等待状态）。

**HIL的不可替代性**：水电站HIL的被测对象是完整的调速器控制柜（含PLC、HMI、通信模块），物理过程（水力-机械-电气系统）由实时仿真器以$\Delta t \leq 1\,\text{ms}$的步长实时模拟，通过真实I/O接口交互。HIL能够发现以下类问题：导叶伺服机构的液压非线性（仅用数学模型无法精确描述）、控制柜电源波动对PLC寄存器的影响（只在真实硬件上出现）。这类问题一旦在现场发生，后果可能是导叶卡死或超速飞车。

> **工程规范**：根据IEC 62061和本系列T3卷的要求，WNAL L3（有条件自主运行）的水利控制系统，必须完成覆盖率不低于80%的HIL测试，包括至少20个极端工况场景（如水头跌落20%、负荷突增30%、单机跳闸等）。SIL测试的代码覆盖率要求不低于MC/DC（修正条件/判定覆盖），这与航空电子AS DO-178C的要求对齐。
...
- 水电站控制系统面临水力-机械-电气三域强耦合、多时间尺度跨越（毫秒级电气暂态至秒级水力暂态）和多目标约束的三重挑战，传统单域PID无法满足全局最优需求
- CPSS框架将信息系统（SCADA/传感器网络）、物理系统（水轮机-发电机组）和社会系统（调度规程、运行人员）整合为统一的控制闭环，三层之间通过标准化接口实现双向交互
- 调速系统的传递函数分析揭示了引水系统刚性水击效应与机组转速响应之间的相位关系，是PID参数整定的理论依据
- 励磁控制中的电压稳定性与有功-无功协调问题须在PSS（电力系统稳定器）框架下统一处理
- CPSS框架下的层级控制策略（机组级-站级-电网级）实现了从毫秒响应到分钟调度的全尺度协同控制
