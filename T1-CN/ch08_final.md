...
水电站作为复杂的水力-机械-电气耦合系统,其控制问题长期面临以下挑战:
...
传统控制方法(如PID控制)虽然在工程中广泛应用,但缺乏统一的理论框架来系统性地处理这些挑战。
...
**问题分析**:
1. **响应滞后**:PID控制器未考虑水击延迟($T_w=2.5$s),导致响应滞后
...
回顾第一章，**水系统控制论（CHS, Cybernetics of Hydro Systems）**提出了水利工程从第四代（数据驱动+智能决策）向第五代（自主运行+人机共融）演进的路径。CHS的核心是建立统一的理论框架，支持水利工程在不同场景下实现自主运行。

**CPSS（Cyber-Physical-Social System）框架**是CHS理论在水电站控制领域的具体实现。它将CHS的抽象理论转化为可操作的建模和控制方法：

- **CHS**：提供统一的理论语言（八原理、WNAL分级、ODD定义）
- **CPSS**：提供水电站场景下的具体建模框架（三层架构、数学模型、控制算法）
- **HydroOS**：提供软件实现平台（第11章详述）

#### (b) CPSS与HDC的关系

**分层分布式控制（HDC, Hierarchical Distributed Control）**是CHS的总体控制架构，适用于所有类型的水利工程（水库、渠道、泵站、水电站等）。CPSS是HDC在水电站场景下的细化：

| HDC通用架构 | CPSS水电站实现 | 时间尺度 | 功能 |
|------------|---------------|---------|------|
...
**水网自主等级（WNAL, Water Network Autonomy Level）**是CHS提出的六级分类体系（L0-L5），类似自动驾驶的SAE分级。CPSS框架支持水电站从L1向L3跨越：

| WNAL等级 | 控制模式 | CPSS实现程度 |
|---------|---------|-------------|
| L0 | 无自动化 | 不适用 |
| L1 | 辅助控制（传统PID） | 仅Physical层 |
| **L2** | **部分自主（本章重点）** | **Physical+Cyber层** |
| **L3** | **条件自主（需ODD验证）** | **Physical+Cyber+Social层** |
| L4 | 高度自主 | 需结合认知智能（第11章） |
| L5 | 完全自主 | 未来目标 |

**L2→L3的关键**：
- L2：CPSS框架提供三层建模和控制算法
- L3：需要通过**在环测试（xIL）**验证**运行设计域（ODD）**（第9章详述）
...
该框架不是简单的分层控制，而是通过**统一的数学语言**（状态空间、传递函数、李雅普诺夫函数）将三个层次有机融合。
...
**CPSS框架的理论贡献**：
- 首次将CPS（Cyber-Physical System）扩展为CPSS，引入Social层
- 建立了水电站三域统一建模的数学框架
- 提供了从L1向L2/L3演进的可行路径

![图8-1: CPSS三层架构图](./figures/ch08_cpss_architecture.png)
...
CPSS框架采用**能量函数**作为统一的分析工具。定义系统总能量:
...
$$
V_h = \frac{1}{2} A_s g z_s^2 + \frac{1}{2} \rho L A q^2
$$

- 第一项:调压室势能($A_s$为截面积,$z_s$为水位偏差)
- 第二项:引水管道动能($\rho$为水密度,$L$为管长,$A$为截面积,$q$为流量)

**推导过程**:
调压室水位$z_s$对应的势能为$mgh = \rho A_s z_s \cdot g \cdot \frac{z_s}{2} = \frac{1}{2} A_s g z_s^2$(取平均高度$\frac{z_s}{2}$)。

引水管道中流速为$v = q/A$,动能为$\frac{1}{2} m v^2 = \frac{1}{2} (\rho L A) \left(\frac{q}{A}\right)^2 = \frac{1}{2} \rho L A q^2$。
...
对于非线性系统$\dot{\mathbf{x}} = \mathbf{f}(\mathbf{x}, \mathbf{u})$,构造李雅普诺夫函数$V(\mathbf{x})$:
...
$$
V = \frac{1}{2} A_s g z_s^2 + \frac{1}{2} J \omega^2 + \int_0^{\delta} (P_e - P_m) d\delta'
$$
...
&= A_s g z_s (q_t - q) - \frac{P_m - P_e}{\omega_0} \omega + (P_e - P_m) \omega \\
&= A_s g z_s (q_t - q) + (P_e - P_m) \left(\omega - \frac{\omega}{\omega_0}\right)
\end{aligned}
$$

在平衡点附近,$\omega \approx \omega_0$,因此:

$$
\dot{V} \approx A_s g z_s (q_t - q) \leq 0
$$

这表明系统能量不断减少,最终收敛到平衡点。
...
$$
\begin{cases}
\frac{\partial h}{\partial t} + \frac{a^2}{g} \frac{\partial q}{\partial x} = 0 \\
\frac{\partial q}{\partial t} + g \frac{\partial h}{\partial x} + \frac{f q|q|}{2DA} = 0
\end{cases}
$$
...
$$
T_m = \frac{P_m}{\omega} = \frac{9.81 \rho g q h \eta_t}{\omega}
$$
...
这是一个简化的**静态励磁系统**模型(IEEE Type 1)。实际励磁系统可能包含更多细节(限幅、稳定器等)。
...
![图8-2: 多物理场耦合示意图](./figures/ch08_multiphysics_coupling.png)

**图8-2**: 水电站系统的多物理场耦合。水力系统（蓝色）、机械系统（绿色）、电气系统（橙色）通过状态变量相互耦合，形成统一的7阶动态系统。
...
**实际应用**:由于水轮机模型的复杂性(非最小相位特性),完全反馈线性化可能导致内部动态不稳定,因此实际中常采用**部分反馈线性化**或**近似线性化**。
...
**稳定性保证**:
- 终端权重$P$满足Lyapunov方程:$\mathbf{A}^T P + P \mathbf{A} - P \mathbf{B} R^{-1} \mathbf{B}^T P + Q = 0$
...
**工程应用**:水电系统中常用递推最小二乘(RLS)或扩展卡尔曼滤波(EKF)实现在线参数辨识。
...
$$
\min J = \int_0^T \left[ C_{fuel}(P) + C_{wear}(\Delta P) + C_{penalty}(\Delta f) + C_{env}(E) \right] dt
$$

- $C_{fuel}(P)$:燃料成本(对于水电站为水资源机会成本)
...
其中$J_{tracking} = \int_0^t (P(\tau) - P^*(\tau))^2 d\tau$。
...
| 层级 | 硬件 | 周期 | 功能 |
|------|------|------|------|
| Physical | PLC/DCS | 1-10ms | 底层控制(导叶、励磁) |
| Cyber | 工控机/边缘计算 | 100ms-1s | 中层控制(AGC、PSS) |
| Social | 服务器/云平台 | 1min-1h | 顶层优化(调度、协调) |
...
某地区风电场(200MW)与水电站(300MW)组成互补系统,应对风电波动。
...
![图8-4: 性能对比曲线](./figures/ch08_performance_comparison.png)
...
4. **第13章（沙坪水电站案例）**:
   - CPSS框架在实际梯级水电站中的应用
   - 验证CPSS在多机组协调、AGC响应、经济调度中的有效性

5. **第14章（胶东调水工程案例）**:
   - CPSS框架可扩展到泵站系统（将水轮机模型替换为水泵模型）
   - 验证CPSS的跨场景适用性

CPSS框架为水电系统的智能化、网络化、绿色化发展提供了理论基础和技术路径，是CHS理论从第四代向第五代演进的关键技术之一，也是实现"双碳"目标的重要支撑。
...
[1] Fang C, Marquez H J. Cyber-Physical Systems: Modeling, Control, and Optimization. Springer, 2020.
...
[4] Slotine J J E, Li W. Applied Nonlinear Control. Prentice Hall, 1991.

[5] Khalil H K. Nonlinear Systems (3rd Edition). Prentice Hall, 2002.

[6] Åström K J, Hägglund T. Advanced PID Control. ISA-The Instrumentation, Systems, and Automation Society, 2006.

[7] Camacho E F, Alba C B. Model Predictive Control (2nd Edition). Springer, 2007.

[8] Sutton R S, Barto A G. Reinforcement Learning: An Introduction (2nd Edition). MIT Press, 2018.

[9] IEEE Working Group. Hydraulic turbine and turbine control models for system dynamic studies. IEEE Transactions on Power Systems, 1992, 7(1): 167-179.

[10] 沈祖诒. 水轮机调节(第3版). 中国水利水电出版社, 2009.

[11] 王飞跃. 平行系统方法与复杂系统的管理和控制. 控制与决策, 2004, 19(5): 485-489.

[12] 李德毅, 王飞跃. 关于复杂系统的哲学思考. 复杂系统与复杂性科学, 2005, 2(1): 1-8.
