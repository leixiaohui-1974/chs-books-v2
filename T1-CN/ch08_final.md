...
> **工程规范**：根据 IEC 62061 和本系列 《标治》 卷的要求，WNAL L3（有条件自主运行）的水利控制系统，必须完成覆盖率不低于 80%的 HIL 测试，包括至少 20 个极端工况场景（如水头跌落 20%、负荷突增 30%、单机跳闸等）。SIL 测试的代码覆盖率要求不低于 MC/DC（修正条件/判定覆盖），这与航空电子 AS DO-178C 的要求对齐。
...
3. **超阈值处理**：若卡涩程度超过预设阈值（如导叶实际开度与指令偏差 $> 5\%$ 持续 $> 10$ s），触发**最小风险状态（MRC）**——自动降负荷至空载或停机状态，进入人工接管流程，由运维人员现场处置
...
> **本章后续进阶**：本章建立了CPSS三空间框架的统一形式化。CPSS框架在水电站场景下的建模细节见ch09；以CPSS视角理解WNAL的决策权渐进让渡见ch10 §10.2.3；CPSS在国家水网全生命周期中的应用见 `_shared/` 目录下的国家水网CPSS论文（codex）。《平台》卷从平台实现角度展开了CPSS在HydroOS三层架构中的工程落地。
