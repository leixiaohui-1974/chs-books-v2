...
> **读者提示**：§8.1以水电站为引导案例展示了CPSS框架的具体应用。但CPSS的价值远不止于水电站——它是描述所有水系统运行的统一框架。任何水系统，无论是单站水电站、梯级枢纽、长距离调水干线还是城市供水管网，其运行都涉及Physical空间（水物理过程）、Cyber空间（模型与控制）和Social空间（目标与规程）的耦合。以下四节从统一视角展开这三个空间的交互机制，为后续§8.2-§8.7的水电站具体实现提供更宽广的理论基座。

### 8.1.5 CPSS三空间与三重反馈回路

传统控制论只关注一个反馈回路：传感器→控制器→执行器→被控对象→传感器。这对应的是Physical↔Cyber之间的闭环。然而，水系统的运行实际上嵌套着**三重反馈回路**，分别作用于截然不同的时间尺度：
...
三重反馈回路的时间尺度相差数个量级，但它们耦合在一起。一个典型的耦合场景：回路3中社会需求变化（城镇化导致供水需求增长）改变了回路2的目标函数（供水保证率从95%提高到97%），继而要求回路1采用更精细的控制策略（从PID切换到MPC）。**水系统复杂性的真正来源，不是物理过程本身——Saint-Venant方程已有200年历史——而是三个空间的耦合反馈。**

传统水利工程的智能化转型之所以频繁出现"单点优秀、全局失配"的困境（见第三章引导案例），根源正在于此：大多数项目只优化了回路1（自动控制），却忽视了回路2（人机协同机制）和回路3（社会需求演化的适应能力）。CHS通过CPSS框架将三者纳入统一视野，为系统级优化提供了理论基础。

### 8.1.6 确定性工作流与灵活工作流

三重反馈回路揭示了一个工程设计上的关键区分：**不同空间对工作流的确定性要求截然不同。**

**Physical空间要求确定性工作流。** 闸门的操作序列必须严格遵循时序（先开退水闸再关节制闸，不可乱序）；泵站的启停必须满足水锤防护的操作间隔；管网的压力管理必须遵循质量守恒和能量守恒。物理因果律不可违背，任何工作流步骤的缺失或乱序都可能导致工程事故。

**Social空间允许灵活工作流。** 调度方案的比选可以根据新信息随时调整；会商讨论可以中断、改序、引入新参与者；应急预案的启动条件可以由调度员根据现场情况灵活判断。人类决策本质上是情境依赖的。

**Cyber空间介于两者之间。** 算法的内部执行流程是确定性的（MPC的优化求解步骤不可跳过），但策略选择可以自适应（根据当前工况切换控制模式、调整预测时域）。

这一区分对AI Agent在水系统中的应用具有直接指导意义。当前AI领域流行的工作流概念可以精确映射到这一谱系：DAG Workflow（有向无环图工作流）对应确定性工作流，Dynamic Routing（动态路由）对应灵活工作流，Agent Loop（如ReAct范式的"推理-行动-观测"循环）对应时间步内的闭环控制。**工作流的选择不是技术偏好，而是被控对象的物理特性决定的**——Physical空间的操作必须用确定性工作流，Social空间的决策可以用灵活工作流，混用将导致系统性风险。
...
**当前的四预实践大多采用开环串行模式**：先跑预报模型出预报结果，再根据预报判断是否预警，再把预报结果放进仿真模型做预演，最后从预案库中选择匹配的方案。四个环节依次传递、单向流动，本质上是§8.1.7所述的开环串行流水线。

**基于CHS的四预闭环重构**，要求四个环节在每个时间步内耦合迭代：

- **预报**（Prediction）= Sensing + Digital Twin 的前向推演。不是跑完全时段再交结果，而是每个时间步同化最新观测、更新预报，并把预报不确定性实时传递给下游环节。
- **预警**（Warning）= ODD状态评估。不是预报之后的独立步骤，而是预报过程的伴生产物——每个时间步实时评估系统是否接近ODD边界，预警级别变化时反过来影响预报策略（加密观测、切换模型、缩短步长）。
- **预演**（Rehearsal）= XIL验证。不是"把预报结果做成动画"，而是在每个决策时间步内快速执行模型在环（MIL）验证：多方案并行仿真，筛选可行域，确认候选方案不违反安全包络。对于关键场景，还需要软件在环（SIL）和调度员在环（OIL）验证。
- **预案**（Plan）= Control 的实时决策输出。不是事先编好的静态方案库，而是每个时间步的闭环优化结果——它随状态更新而更新，是活的、动态的。

四预闭环体系在不同时间尺度上表现不同。**宏观尺度**（天至周）以灵活工作流为主，Social空间主导目标设定，四预表现为方案比选和滚动修正。**实时尺度**（分钟至小时）以确定性工作流为主，Physical空间主导时序要求，四预表现为每个时间步内的闭环耦合——预报模型、预警评估、XIL预演、决策输出在一个控制周期内完成迭代。

在实际运行中，水系统往往在多种运行模态之间切换：正常工况（G0）下四预以宏观尺度为主；当系统检测到异常（如水质事件、局部暴雨、设备故障），运行模态升级（G1注意→G2警戒→G3应急），四预的实时尺度闭环随之加密——同化频率提高、预测时域缩短、安全约束收紧。这种模态驱动的四预策略切换，正是§8.1.6所述确定性/灵活工作流动态组合的工程体现。
