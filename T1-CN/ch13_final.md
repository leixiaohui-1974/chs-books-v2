...
> 图13-1: HydroOS 三层架构图
{描述：三层垂直堆叠（设备抽象层 DAL → 物理AI引擎层 HydroCore → 认知AI引擎层 HydroClaw），层间双向箭头。}
{尺寸：全页}
...
{描述：三层叠加（底层 SCADA→中层 HydroOS DAL+HydroCore→上层 HydroClaw+ 治理），标注数据流和控制流。}
{尺寸：全页}
...
某调水干线全长约 500 公里，沿线设 10 余座泵站和数十座闸站，已部署 SCADA 系统。核心需求：统一设备语义、实现梯级联合优化调度、增强人机协同。

**DAL 层**：完成 1000+ 设备的 UDSM 注册，支持 Modbus、OPC UA 等协议适配，数据可用率从 95% 提升至 99%+。关键经验：设备抽象层是整个架构的基础，协议适配器的质量直接决定上层系统的稳定性。

**HydroCore 层**：建立 IDZ 降阶模型覆盖全工程，MPC 控制器预测时域 12 小时，安全包络三区约束嵌入优化。水位控制精度从±15 cm 提升至±5 cm★，应急响应时间从 15 分钟缩短至 5 分钟★。关键经验：物理 AI 引擎直接决定控制性能，模型精度是核心瓶颈。

**HydroClaw 层**：构建领域知识图谱，微调大语言模型支持智能问答和预案推荐。关键经验：认知 AI 引擎是增强能力而非必需能力，应在 HydroCore 稳定运行后再引入。
