...
3. **判别**系统是否满足Thoma稳定条件，并解释不满足时的振荡风险；
4. **整定**面向水力时滞与惯性特征的PID参数，并比较不同整定准则的效果；
5. **设计**Smith预测补偿结构以削弱纯滞后影响，并评估其对跟踪与抗扰性能的提升。
...
令$K = A_s^{-1}$、$X = (\tau_d - \tau_m)/K$，两式的**分子结构**完全一致——都含有一个右半平面零点，反映了扩散波的非最小相位特性。但分母结构存在根本差异：IDZ为积分型（$A_s \cdot s$），Muskingum为自调节型（$1 + K(1-X)s$）。物理上，这对应于边界条件的差异：IDZ描述的是**固定闸门下游水位响应**（水位可无限积累），而Muskingum描述的是**自由出流河段的流量传播**（出流随水位自调节）。尽管如此，在Nash级联极限（Lemma 2）下，当Muskingum级联数$n \to \infty$且$nK \to \tau_d$时，级联传递函数趋向于含纯延迟的积分器，两族传递函数在此极限意义下统一。这说明：**经典水文学中的Muskingum河道演算方法与控制论中的IDZ传递函数模型，源自同一物理过程（扩散波），是不同边界条件下的互补描述**。
...
2. **参数时变的影响**：
   ...
   - 当流量变化幅度较大（如±30%）时，对应时滞变化可超过±20%，此时Smith预估器性能可能下降20%–40%
...
- 明渠传递函数、有压管道传递函数和水电站调速传递函数尽管物理背景不同，均可纳入统一的时滞-惯性环节框架，体现了CHS结构同构性命题
- 频域分析工具（波特图、奈奎斯特图）在水系统控制器设计中的应用：通过幅频特性确定PID参数的比例增益上界，通过相频特性确定时滞补偿的必要性
- 数据驱动辨识方法（频率响应辨识、PRBS激励）可在运行工况下在线更新传递函数参数，克服物理模型参数不确定性
- 统一传递函数族为后续章节（MPC设计、安全包络分析、WNAL分级评估）提供了共同的数学语言

## 习题

1. 已知某灌溉渠道的圣维南方程经线性化后可得到如下参数：渠道长度 $L = 5$ km，波速 $c = 2$ m/s，正常流量 $Q_0 = 3$ m³/s。试推导该渠道从上游流量扰动到下游水位响应的近似传递函数，并说明纯时滞近似的适用条件。

2. 对比明渠IDZ传递函数 $G_{IDZ}(s) = \frac{K e^{-\tau s}}{s}$ 与有压管道传递函数 $G_{pipe}(s) = \frac{1}{T_w s + 1}$（水击时间常数 $T_w$）在控制器设计上的本质差异：分别讨论两者对PID参数的约束，并说明哪种系统更难控制及原因。

3. 某水电站调速系统的开环传递函数为 $G_0(s) = \frac{(1-T_w s)}{T_a s(1+0.5T_w s)}$（其中 $T_w$ 为水击时间常数，$T_a$ 为机组惯性时间常数）。利用奈奎斯特稳定判据，分析该系统在 $T_w = 1$ s，$T_a = 8$ s 条件下的稳定裕度，并给出PID参数整定的建议。

4. 描述一种使用PRBS（伪随机二进制序列）信号对在线运行渠道进行频率响应辨识的实验方案，包括：激励信号幅值选择依据、采样频率设置原则、数据预处理方法，以及如何验证辨识结果的可信度。
