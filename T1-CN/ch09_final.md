...
**推导来源**：Thoma, D. (1910), "Zur Theorie des Wasserschlosses bei selbsttätig geregelten Turbinenanlagen" (On the Theory of Surge Tanks in Automatically Regulated Turbine Plants), *Oldenbourg*. 该条件基于小扰动线性化分析，确保调压室水位振荡的阻尼为正。

**物理意义**：
- 分子$L q_0$：隧洞水流惯性
- 分母$2 g H_0 \tan\alpha$：隧洞摩阻提供的阻尼
- 条件保证：调压室容积足够大，能够吸收水流惯性引起的振荡
...
- 额定水头: 80 m
- 引水管长度: 200 m
- 惯性时间常数: $T_w = 2.5$ s, $H = 8$ s

> **参数自洽性验证**：根据$T_w = \frac{L v_0}{g H_0}$，可推算额定流速$v_0 = \frac{g H_0 T_w}{L} = \frac{9.8 \times 80 \times 2.5}{200} \approx 9.8$ m/s，符合水电站引水管道的典型流速范围（8-12 m/s）。
...
1. **工况避开**:通过优化调度,避免在S区运行
2. **快速穿越**:加大导叶开度变化速率,快速穿越S区
3. **非线性控制**:采用滑模控制,保证全工况稳定
