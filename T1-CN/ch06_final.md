...
2. **检验**系统模型是否满足Kalman秩条件，并判断不可控/不可观模态；
3. **计算**可控Gramian与可观Gramian，并解释其与能量需求和估计精度的关系；
...
基于 IDZ 传递函数模型 [6-5] 及管光华等 [6-3] 提出的含多分水口渠道广义 ID 建模方法，建立线性状态空间模型。IDZ传递函数对应CHS统一传递函数族（第九章§9.4）中Family α的Level ii近似，其参数 $(A_s, \tau_d, \tau_m)$ 可通过本节在线辨识方法获取。具体模型形式为：
...
> **与 CHS 体系的联系**：大规模系统的可控可观性分解分析是 HydroOS 物理AI引擎层（HydroCore，第十三章）中"状态估计服务"设计的理论依据，决定了哪些状态需要全系统协调估计、哪些状态可在子区域内独立估计。
...
> **术语说明**：HydroOS 三层架构为 DAL（设备抽象层）→ HydroCore（物理AI引擎层）→ HydroClaw（认知AI引擎层），详见第十三章完整定义和 T2b 第九章的软件工程规格。本节从可控可观性理论角度分析各层的能力定位。
...
**物理AI引擎（HydroCore）：可观性估计与可控性执行的核心**
