...
$$\frac{\partial Q}{\partial t} + \frac{\partial}{\partial x}\!\left(\frac{Q^2}{A}\right) + gA\frac{\partial h}{\partial x} + gA(S_f - S_0) = 0 \tag{4.3}$$
...
$$\frac{\partial Q}{\partial t}\bigg|_{i+1/2}^{j+\theta} \approx \frac{1}{2\Delta t}\left[(Q_{i+1}^{j+1} - Q_{i+1}^{j}) + (Q_i^{j+1} - Q_i^{j})\right] \tag{4.4}$$
...
$$\alpha_{Fr} = \begin{cases} 1.00, & Fr < 0.5 \ 0.90, & 0.5 \leq Fr < 0.8 \ 0.75, & 0.8 \leq Fr < 1.0 \ 0.50, & Fr \geq 1.0 \end{cases} \tag{4.11}$$
...
$$\mathbf{P}^f_k = \frac{1}{N_e - 1} \sum_{i=1}^{N_e} \left(\mathbf{x}^f_{k,i} - \bar{\mathbf{x}}^f_k\right)\left(\mathbf{x}^f_{k,i} - \bar{\mathbf{x}}^f_k\right)^T \tag{4.14}$$
...
$$\mathbf{K}_k = \mathbf{P}^f_k \mathbf{H}^T \left(\mathbf{H} \mathbf{P}^f_k \mathbf{H}^T + \mathbf{R}_{\text{noise}}\right)^{-1} \tag{4.16}$$

**状态更新**（分析步）：

$$\mathbf{x}^a_{k,i} = \mathbf{x}^f_{k,i} + \mathbf{K}_k\!\left(\mathbf{y}_k + \boldsymbol{\varepsilon}^o_i - \mathbf{H}\mathbf{x}^f_{k,i}\right) \tag{4.17}$$
...
$$W(\mathbf{x}_k) = \max_{i \in \mathcal{N},\, j \in [1, N_p]} \left\{ \max\!\left(h_{i,k+j|k} - h_i^{\max},\, h_i^{\min} - h_{i,k+j|k},\, 0 \right) \right\} \tag{4.33}$$
...
$$\mathbf{x}^{cf}_{k+j} = f\!\left(\mathbf{x}_{k+j-1},\, u^{cf}_j\right),\quad j = 1, 2, \ldots, N_p \tag{4.35}$$

情景库 $\mathcal{S}_k$ 定义为：

$$\mathcal{S}_k = \left\{ \left(\mathbf{U}^{(s)},\, \mathbf{X}^{(s)}\right) : \mathbf{U}^{(s)} \in \mathcal{U}_{\text{sample}} \right\} \tag{4.36}$$
