...
3. **预演**：MPC对多个候选控制序列进行快速MIL预演——用降阶模型评估每个候选方案的后果，筛除违反安全包络的方案，保留可行域内的最优解。这是§4.4将要详述的优化求解过程。
4. **预案**：MPC输出的最优控制序列第一步即为当前步长的可执行指令——预案不是静态文档，而是每个Δt更新一次的动态决策。

**确定性引擎与灵活引擎的双轨设计**　上述步长级四预是HydroOS的**确定性引擎**——步骤固定（同化→预报→预演→预案→执行）、时序严格（必须在Δt内完成）、不可中断。与之并行的是HydroClaw驱动的**灵活引擎**——处理Skill级四预（事件驱动的方案比选、调度规程检索、多方会商），运行在更长的时间尺度上，允许中断和改序。两套引擎通过状态总线共享系统状态，但执行链路独立，确保确定性引擎不受灵活引擎的延迟或异常影响。

**通信中断降级**　当中心通信中断超过阈值时，现地MPC自动切换降级模式：M1模式（保持最后目标+本地自主控制）——现地MPC继续以最后收到的目标轨迹运行确定性四预闭环；M2模式（仅保证本站水位安全）——放弃优化目标，只执行安全包络内的保守控制。这是确定性工作流在极端场景下的兜底设计。
